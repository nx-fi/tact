...
export function storeMultisigContract$Data(src: MultisigContract$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeUint(src.seqno, 32);
        b_0.storeUint(src.user1Key, 256);
        b_0.storeUint(src.user2Key, 256);
        b_0.storeUint(src.user3Key, 256);
    };
}
...
export function loadMultisigContract$Data(slice: Slice) {
    const sc_0 = slice;
    const _seqno = sc_0.loadUintBig(32);
    const _user1Key = sc_0.loadUintBig(256);
    const _user2Key = sc_0.loadUintBig(256);
    const _user3Key = sc_0.loadUintBig(256);
    return { $$type: 'MultisigContract$Data' as const, seqno: _seqno, user1Key: _user1Key, user2Key: _user2Key, user3Key: _user3Key };
}

function loadTupleMultisigContract$Data(source: TupleReader) {
    const _seqno = source.readBigNumber();
    const _user1Key = source.readBigNumber();
    const _user2Key = source.readBigNumber();
    const _user3Key = source.readBigNumber();
    return { $$type: 'MultisigContract$Data' as const, seqno: _seqno, user1Key: _user1Key, user2Key: _user2Key, user3Key: _user3Key };
}
...
function storeTupleMultisigContract$Data(source: MultisigContract$Data) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.seqno);
    builder.writeNumber(source.user1Key);
    builder.writeNumber(source.user2Key);
    builder.writeNumber(source.user3Key);
    return builder.build();
}
...
async function MultisigContract_init(key1: bigint, key2: bigint, key3: bigint) {
    const __code = Cell.fromBase64('');
...
const MultisigContract_types: ABIType[] = [
    ...
    {"name":"MultisigContract$Data","header":null,"fields":[{"name":"seqno","type":{"kind":"simple","type":"uint","optional":false,"format":32}},{"name":"user1Key","type":{"kind":"simple","type":"uint","optional":false,"format":256}},{"name":"user2Key","type":{"kind":"simple","type":"uint","optional":false,"format":256}},{"name":"user3Key","type":{"kind":"simple","type":"uint","optional":false,"format":256}}]},
]
...
export class MultisigContract implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: Slice | Execute) {
