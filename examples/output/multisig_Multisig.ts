...
export function loadRequest(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 4266760323) {
...
export function loadSigned(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2213172633) {
        throw Error("Invalid prefix");
    }
...
export function loadMultisigSigner$Data(slice: Slice) {
    ...
    const _members = Dictionary.load(
        Dictionary.Keys.Address(),
        Dictionary.Values.BigInt(257),
        sc_0,
    );
    ...
    return {
        $$type: "MultisigSigner$Data" as const,
        master: _master,
        members: _members,
        weight: _weight,
...
function loadTupleMultisigSigner$Data(source: TupleReader) {
    ...
    const _members = Dictionary.loadDirect(
        Dictionary.Keys.Address(),
        Dictionary.Values.BigInt(257),
        source.readCellOpt(),
    );
...
function dictValueParserMultisigSigner$Data(): DictionaryValue<MultisigSigner$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeMultisigSigner$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
function dictValueParserMultisig$Data(): DictionaryValue<Multisig$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeMultisig$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
async function Multisig_init(
    ...
    const __code = Cell.fromBase64(
        "",
    );
    const builder = beginCell();
    builder.storeUint(0, 1);
    initMultisig_init_args({
        $$type: "Multisig_init_args",
        members,
        totalWeight,
        requiredWeight,
...
const Multisig_types: ABIType[] = [
    ...
    },
    {
        ...
        fields: [
            {
                name: "request",
                type: { kind: "simple", type: "Request", optional: false },
            },
        ],
    },
    {
        name: "MultisigSigner$Data",
        header: null,
        fields: [
            {
                name: "master",
...
export class Multisig implements Contract {
    ...
    static async fromInit(
        ...
        const __gen_init = await Multisig_init(
            members,
            totalWeight,
            requiredWeight,
        );
    ...
    async getMember(provider: ContractProvider, address: Address) {
        ...
        const source = (await provider.get(70789 as any, builder.build()))
            .stack;
    ...
    async getMembers(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(104769 as any, builder.build()))
            .stack;
