...
export function loadToggle(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 1081595080) {
        throw Error("Invalid prefix");
    }
    const _key = sc_0.loadIntBig(257);
...
export function loadPersist(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3801943978) {
        throw Error("Invalid prefix");
    }
    const _key = sc_0.loadIntBig(257);
...
export function loadReset(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 1718153138) {
        throw Error("Invalid prefix");
    }
    const _key = sc_0.loadIntBig(257);
...
export function loadIncrementContract$Data(slice: Slice) {
    const sc_0 = slice;
    const _counters = Dictionary.load(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.BigInt(257),
        sc_0,
    );
    const _counters2 = Dictionary.load(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.Bool(),
        sc_0,
    );
    const sc_1 = sc_0.loadRef().beginParse();
    const _counters3 = Dictionary.load(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.Cell(),
        sc_1,
    );
    const _counters4 = Dictionary.load(
        Dictionary.Keys.Address(),
        Dictionary.Values.BigInt(257),
        sc_1,
    );
    const _counters5 = Dictionary.load(
        Dictionary.Keys.BigInt(257),
        dictValueParserSomething(),
        sc_1,
    );
    return {
        $$type: "IncrementContract$Data" as const,
        counters: _counters,
        counters2: _counters2,
        counters3: _counters3,
        counters4: _counters4,
        counters5: _counters5,
    };
}

function loadTupleIncrementContract$Data(source: TupleReader) {
    const _counters = Dictionary.loadDirect(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.BigInt(257),
        source.readCellOpt(),
    );
    const _counters2 = Dictionary.loadDirect(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.Bool(),
        source.readCellOpt(),
    );
    const _counters3 = Dictionary.loadDirect(
        Dictionary.Keys.BigInt(257),
        Dictionary.Values.Cell(),
        source.readCellOpt(),
    );
    const _counters4 = Dictionary.loadDirect(
        Dictionary.Keys.Address(),
        Dictionary.Values.BigInt(257),
        source.readCellOpt(),
    );
    const _counters5 = Dictionary.loadDirect(
        Dictionary.Keys.BigInt(257),
        dictValueParserSomething(),
        source.readCellOpt(),
    );
...
function dictValueParserIncrementContract$Data(): DictionaryValue<IncrementContract$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeIncrementContract$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
async function IncrementContract_init() {
    const __code = Cell.fromBase64(
        "",
    );
    const builder = beginCell();
    builder.storeUint(0, 1);
    initIncrementContract_init_args({ $$type: "IncrementContract_init_args" })(
        builder,
    );
...
export class IncrementContract implements Contract {
    ...
    async send(
        ...
        message: Increment | Toggle | Persist | Reset | Deploy,
    ) {
    ...
    async getCounters(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(95239 as any, builder.build()))
            .stack;
    ...
    async getCounters2(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(77570 as any, builder.build()))
            .stack;
        const result = Dictionary.loadDirect(
            Dictionary.Keys.Address(),
            Dictionary.Values.BigInt(257),
