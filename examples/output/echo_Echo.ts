...
export function loadEchoMessage(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2074217921) {
...
function dictValueParserEchoMessage(): DictionaryValue<EchoMessage> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeEchoMessage(src)).endCell(),
            );
        },
        parse: (src) => {
...
async function Echo_init(a: bigint) {
    const __code = Cell.fromBase64(
        "",
    );
...
export class Echo implements Contract {
    ...
    static async fromInit(a: bigint) {
        const __gen_init = await Echo_init(a);
    ...
    async send(
        ...
        message: EchoMessage | string | Slice,
    ) {
    ...
    async getHello(provider: ContractProvider, src: string) {
        ...
        const source = (await provider.get(115554 as any, builder.build()))
            .stack;
    ...
    async getHello2(provider: ContractProvider, src: bigint) {
        ...
        const source = (await provider.get(105790 as any, builder.build()))
            .stack;
