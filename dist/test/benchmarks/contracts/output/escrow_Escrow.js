...
exports.storeFunding = storeFunding;
exports.loadFunding = loadFunding;
exports.storeApprove = storeApprove;
exports.loadApprove = loadApprove;
exports.storeCancel = storeCancel;
exports.loadCancel = loadCancel;
exports.storeProvideEscrowData = storeProvideEscrowData;
exports.loadProvideEscrowData = loadProvideEscrowData;
...
function loadUpdateJettonWalletCode(slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2662129297) {
        throw Error('Invalid prefix');
    }
...
function storeFunding(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeUint(2488396969, 32);
    };
}
function loadFunding(slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2488396969) {
        throw Error('Invalid prefix');
    }
    return { $$type: 'Funding' };
}
function loadTupleFunding(source) {
    return { $$type: 'Funding' };
}
function loadGetterTupleFunding(source) {
    return { $$type: 'Funding' };
}
function storeTupleFunding(source) {
    const builder = new core_1.TupleBuilder();
    return builder.build();
}
function dictValueParserFunding() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeFunding(src)).endCell());
        },
        parse: (src) => {
...
function storeApprove(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeUint(3904984705, 32);
    };
}
function loadApprove(slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3904984705) {
        throw Error('Invalid prefix');
    }
    return { $$type: 'Approve' };
}
function loadTupleApprove(source) {
    return { $$type: 'Approve' };
}
function loadGetterTupleApprove(source) {
    return { $$type: 'Approve' };
}
function storeTupleApprove(source) {
    const builder = new core_1.TupleBuilder();
    return builder.build();
}
function dictValueParserApprove() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeApprove(src)).endCell());
        },
        parse: (src) => {
...
function storeCancel(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeUint(3423544614, 32);
    };
}
function loadCancel(slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3423544614) {
        throw Error('Invalid prefix');
    }
    return { $$type: 'Cancel' };
}
function loadTupleCancel(source) {
    return { $$type: 'Cancel' };
}
function loadGetterTupleCancel(source) {
    return { $$type: 'Cancel' };
}
function storeTupleCancel(source) {
    const builder = new core_1.TupleBuilder();
    return builder.build();
}
function dictValueParserCancel() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeCancel(src)).endCell());
        },
        parse: (src) => {
...
function storeProvideEscrowData(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeUint(3275436326, 32);
    };
}
function loadProvideEscrowData(slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3275436326) {
        throw Error('Invalid prefix');
    }
    return { $$type: 'ProvideEscrowData' };
}
function loadTupleProvideEscrowData(source) {
    return { $$type: 'ProvideEscrowData' };
}
function loadGetterTupleProvideEscrowData(source) {
    return { $$type: 'ProvideEscrowData' };
}
function storeTupleProvideEscrowData(source) {
    const builder = new core_1.TupleBuilder();
    return builder.build();
}
function dictValueParserProvideEscrowData() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeProvideEscrowData(src)).endCell());
        },
        parse: (src) => {
...
class Escrow {
    static GUARANTOR_PERCENT_CONST = 100000n;
    static GUARANTOR_PERCENT_MAX = 90000n;
    static JETTON_TRANSFER_GAS = 50000000n;
    static TON_TRANSFER_GAS = 15000000n;
    ...
    async send(provider, via, args, message) {
        let body = null;
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'Funding') {
            body = (0, core_1.beginCell)().store(storeFunding(message)).endCell();
        }
        ...
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'Approve') {
            body = (0, core_1.beginCell)().store(storeApprove(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'Cancel') {
            body = (0, core_1.beginCell)().store(storeCancel(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'ProvideEscrowData') {
            body = (0, core_1.beginCell)().store(storeProvideEscrowData(message)).endCell();
        }
        if (body === null) {
