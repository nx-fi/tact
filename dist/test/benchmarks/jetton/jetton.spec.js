...
describe("Jetton", () => {
    ...
    beforeAll(async () => {
        ...
        jettonMinter = blockchain.openContract(await jetton_minter_discoverable_JettonMinter_1.JettonMinter.fromInit(deployer.address, defaultContent));
    ...
    it("transfer", async () => {
        const runMintTest = async (minterAddress) => {
            const mintResult = await sendMintRaw(minterAddress, deployer, deployer.address, (0, core_1.toNano)(100000), (0, core_1.toNano)("0.05"), (0, core_1.toNano)("1"));
            ...
            const sendResult = await sendTransferRaw(deployerJettonWalletAddress, deployer, (0, core_1.toNano)(1), 1n, someAddress, deployer.address, null, 0n, null);
    ...
    it("burn", async () => {
        const runBurnTest = async (minterAddress) => {
            ...
            const burnResult = await sendBurnRaw(deployerJettonWalletAddress, deployer, (0, core_1.toNano)(10), burnAmount, deployer.address, null);
    ...
    it("discovery", async () => {
        const runDiscoveryTest = async (minterAddress) => {
            const discoveryResult = await sendDiscoveryRaw(minterAddress, deployer, notDeployer.address, false, (0, core_1.toNano)(10));
