...
exports.MasterV0 = exports.MasterV0_getterMapping = void 0;
...
exports.storeMasterV0$Data = storeMasterV0$Data;
exports.loadMasterV0$Data = loadMasterV0$Data;
...
function loadTest$Data(slice) {
    ...
    return { $$type: 'Test$Data', owner: _owner };
}
...
function storeMasterV0$Data(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeInt(src.state, 257);
    };
}
function loadMasterV0$Data(slice) {
    const sc_0 = slice;
    const _state = sc_0.loadIntBig(257);
    return { $$type: 'MasterV0$Data', state: _state };
}
function loadTupleMasterV0$Data(source) {
    const _state = source.readBigNumber();
    return { $$type: 'MasterV0$Data', state: _state };
}
function loadGetterTupleMasterV0$Data(source) {
    const _state = source.readBigNumber();
    return { $$type: 'MasterV0$Data', state: _state };
}
function storeTupleMasterV0$Data(source) {
    const builder = new core_1.TupleBuilder();
    builder.writeNumber(source.state);
    return builder.build();
}
function dictValueParserMasterV0$Data() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeMasterV0$Data(src)).endCell());
        },
        parse: (src) => {
...
function initMasterV0_init_args(src) {
    return (builder) => {
        const b_0 = builder;
    };
}
async function MasterV0_init() {
    const __code = core_1.Cell.fromBase64('te6ccgEBAwEAXwABev8AII4vMDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABl4EBAdcAATGSMHDiMNzywILh9KQT9LzyyAsBATGmS8V7UTQ0gABl4EBAdcAATGSMHDi2zwxgAgACIA==');
...
class MasterV0 {
    ...
    async getState(provider) {
        const builder = new core_1.TupleBuilder();
        const source = (await provider.get(77589, builder.build())).stack;
        const result = source.readBigNumber();
        return result;
    }
}
exports.MasterV0 = MasterV0;
