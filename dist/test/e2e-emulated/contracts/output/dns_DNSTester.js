...
exports.DNSTester = exports.DNSTester_getterMapping = void 0;
...
exports.storeDNSTester$Data = storeDNSTester$Data;
exports.loadDNSTester$Data = loadDNSTester$Data;
...
function storeDNSTester$Data(src) {
    return (builder) => {
        const b_0 = builder;
        b_0.storeRef(src.tmpC);
    };
}
function loadDNSTester$Data(slice) {
    const sc_0 = slice;
    const _tmpC = sc_0.loadRef();
    return { $$type: 'DNSTester$Data', tmpC: _tmpC };
}
function loadTupleDNSTester$Data(source) {
    const _tmpC = source.readCell();
    return { $$type: 'DNSTester$Data', tmpC: _tmpC };
}
function loadGetterTupleDNSTester$Data(source) {
    const _tmpC = source.readCell();
    return { $$type: 'DNSTester$Data', tmpC: _tmpC };
}
function storeTupleDNSTester$Data(source) {
    const builder = new core_1.TupleBuilder();
    builder.writeCell(source.tmpC);
    return builder.build();
}
function dictValueParserDNSTester$Data() {
    return {
        serialize: (src, builder) => {
            builder.storeRef((0, core_1.beginCell)().store(storeDNSTester$Data(src)).endCell());
        },
        parse: (src) => {
...
function initDNSTester_init_args(src) {
    return (builder) => {
        const b_0 = builder;
    };
}
async function DNSTester_init() {
    const __code = core_1.Cell.fromBase64('');
...
exports.DNSTester_getterMapping = {
...
class DNSTester {
    ...
    async getStringToInternal(provider, src) {
        const builder = new core_1.TupleBuilder();
        builder.writeString(src);
        const source = (await provider.get(97933, builder.build())).stack;
        const result = source.readCellOpt()?.asSlice() ?? null;
        return result;
    }
    async getInternalNormalize(provider, src) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(src.asCell());
        const source = (await provider.get(114271, builder.build())).stack;
        const result = source.readCell().asSlice();
        return result;
    }
    async getDnsInternalVerify(provider, subdomain) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(subdomain.asCell());
        const source = (await provider.get(74415, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    async getDnsExtractTopDomainLength(provider, subdomain) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(subdomain.asCell());
        const source = (await provider.get(114191, builder.build())).stack;
        const result = source.readBigNumber();
        return result;
    }
    async getDnsExtractTopDomain(provider, subdomain) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(subdomain.asCell());
        const source = (await provider.get(103737, builder.build())).stack;
        const result = source.readCell().asSlice();
        return result;
    }
    async getDnsresolve(provider, subdomain, category) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(subdomain.asCell());
        builder.writeNumber(category);
        const source = (await provider.get(123660, builder.build())).stack;
