...
class StdlibTest {
    ...
    async getLoadMaybeRef(provider, c) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(c.asCell());
        const source = (await provider.get(87792, builder.build())).stack;
        const result = source.readCellOpt();
        return result;
    }
    async getPreloadMaybeRef(provider, c) {
        const builder = new core_1.TupleBuilder();
        builder.writeSlice(c.asCell());
        const source = (await provider.get(73300, builder.build())).stack;
        const result = source.readCellOpt();
        return result;
    }
