...
const maps1_MapTestContract_1 = require("./contracts/output/maps1_MapTestContract");
const sandbox_1 = require("@ton/sandbox");
const core_1 = require("@ton/core");
require("@ton/test-utils");
// Type Guard for SomeStruct
function isSomeStruct(value) {
    return (typeof value === "object" &&
        value !== null &&
        "$$type" in value &&
        value.$$type === "SomeStruct" &&
        "int" in value &&
        "bool" in value &&
        "address" in value &&
        "a" in value &&
        "b" in value &&
        typeof value.int === "bigint" &&
        typeof value.bool === "boolean" &&
        value.address instanceof core_1.Address &&
        typeof value.a === "bigint" &&
...
const testCases = [
    {
        keys: {
            ...
            keyUint8: 200n,
            keyUint42: 500000n,
            keyUint256: 1000000000000n,
            keyAddress: (0, random_utils_1.randomAddress)(0, "address0"),
        },
        values: {
            valueVarint16: 123n,
            valueVarint32: 123456n,
            valueVaruint16: 255n,
            valueVaruint32: 123456789n,
            valueBool: true,
            valueCell: (0, core_1.beginCell)().storeUint(42, 32).endCell(),
            valueAddress: (0, random_utils_1.randomAddress)(0, "address"),
            valueStruct: {
                $$type: "SomeStruct",
                int: 321n,
                bool: false,
                address: (0, random_utils_1.randomAddress)(0, "address"),
                a: 10n,
                b: -20n,
            },
        },
    },
    {
        keys: {
            ...
            keyAddress: (0, random_utils_1.randomAddress)(0, "address1"),
        },
        values: {
    ...
    },
    {
        keys: {
            ...
            keyUint42: 0n,
            keyUint256: 0n,
            keyAddress: (0, random_utils_1.randomAddress)(0, "address2"),
        },
        values: {
            valueVarint16: -1n,
            valueVarint32: -1n,
            ...
            valueVaruint32: 1n,
            valueBool: false,
            valueCell: (0, core_1.beginCell)().storeUint(0, 32).endCell(),
            valueAddress: (0, random_utils_1.randomAddress)(0, "address"),
            valueStruct: {
                $$type: "SomeStruct",
                int: 0n,
    ...
    },
    {
        keys: {
            ...
            keyAddress: (0, random_utils_1.randomAddress)(0, "address3"),
        },
        values: {
...
describe("MapTestContract", () => {
    ...
    beforeEach(async () => {
        ...
        contract = blockchain.openContract(await maps1_MapTestContract_1.MapTestContract.fromInit());
        // Fund the contract with some TONs
        await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("10") }, null);
        // Check that all maps are empty initially
        const maps = await contract.getAllMaps();
        for (const [_mapName, map] of Object.entries(maps)) {
            if (map instanceof core_1.Dictionary) {
    ...
    it("set: should set and clear values", async () => {
        for (const { keys, values } of testCases) {
            ...
            await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, setMessage);
            ...
            mapConfigs.forEach(({ mapName, key, value, keyTransform, valueTransform }) => {
                ...
                if (expectedValue instanceof core_1.Cell) {
                    expect(actualValue).toEqualCell(expectedValue);
                }
                else if (expectedValue instanceof core_1.Address) {
                    expect(actualValue).toEqualAddress(expectedValue);
                }
                else if (isSomeStruct(expectedValue)) {
                    expect(compareStructs(actualValue, expectedValue)).toBe(true);
                }
                else {
            ...
            await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, clearMessage);
    ...
    it("get: should get values after setting them and nulls after clearing", async () => {
        for (const { keys, values } of testCases) {
            ...
            const getResponse = await contract.getGetAllMaps(keys.keyInt, keys.keyInt8, keys.keyInt42, keys.keyInt256, keys.keyUint8, keys.keyUint42, keys.keyUint256, keys.keyAddress);
            // Iterate over mapConfigs and perform assertions
            mapConfigs.forEach(({ mapName, key: _key, value, keyTransform: _keyTransform, valueTransform, }) => {
                let expectedValue = values[value];
            ...
            const clearedGetResponse = await contract.getGetAllMaps(keys.keyInt, keys.keyInt8, keys.keyInt42, keys.keyInt256, keys.keyUint8, keys.keyUint42, keys.keyUint256, keys.keyAddress);
            // Iterate over mapConfigs and assert maps are empty
            mapConfigs.forEach(({ mapName }) => {
    ...
    it("get: should return null for non-existent keys", async () => {
        ...
        const getResponse = await contract.getGetAllMaps(nonExistentKeys.keyInt, nonExistentKeys.keyInt8, nonExistentKeys.keyInt42, nonExistentKeys.keyInt256, nonExistentKeys.keyUint8, nonExistentKeys.keyUint42, nonExistentKeys.keyUint256, nonExistentKeys.keyAddress);
    ...
    it("del: should delete values", async () => {
        for (const { keys, values } of testCases) {
            ...
            mapConfigs.forEach(({ mapName, key, value, keyTransform, valueTransform }) => {
                const map = allMapsBeforeDel[mapName];
                expect(map.size).toBe(1);
                let mapKey = keys[key];
                if (keyTransform) {
                    mapKey = keyTransform(mapKey);
    ...
    it("del: should delete multiple values", async () => {
        ...
        // Delete them
        for (const { keys } of testCases) {
            const delMessage = { $$type: "DelAllMaps", ...keys };
            await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, delMessage);
        }
    ...
    it("exists: should return 'true' for existing keys and 'false' for non-existent keys", async () => {
        ...
        for (const { keys } of testCases.slice(0, -1)) {
            const existsResponse = await contract.getExistsAllMaps(keys.keyInt, keys.keyInt8, keys.keyInt42, keys.keyInt256, keys.keyUint8, keys.keyUint42, keys.keyUint256, keys.keyAddress);
        ...
        const nonExistentKeys = testCases[testCases.length - 1].keys;
        const nonExistentResponse = await contract.getExistsAllMaps(nonExistentKeys.keyInt, nonExistentKeys.keyInt8, nonExistentKeys.keyInt42, nonExistentKeys.keyInt256, nonExistentKeys.keyUint8, nonExistentKeys.keyUint42, nonExistentKeys.keyUint256, nonExistentKeys.keyAddress);
    ...
    it("asCell: should correctly serialize and deserialize maps", async () => {
        for (const { keys, values } of testCases) {
            ...
            mapConfigs.forEach(({ mapName, key, value, keyTransform, valueTransform }) => {
                ...
                const serializedMap = (0, core_1.beginCell)()
                    .storeDictDirect(map)
                    .endCell();
    ...
    it("replace: should replace values and clear them", async () => {
        for (const { keys } of testCases) {
            ...
            for (const { values } of testCases) {
                ...
                await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, replaceMessage);
                // Retrieve all maps using `allMaps` getter
                const allMaps = await contract.getAllMaps();
                // Iterate over mapConfigs and perform assertions
                mapConfigs.forEach(({ mapName, key, value, keyTransform, valueTransform }) => {
    ...
    it("replace: should return 'true' when replacing values and 'false' when keys do not exist", async () => {
        for (const { keys, values } of testCases) {
            ...
            const replaceResult = await contract.getReplaceAllMaps(keys.keyInt, keys.keyInt8, keys.keyInt42, keys.keyInt256, keys.keyUint8, keys.keyUint42, keys.keyUint256, keys.keyAddress, values.valueVarint16, values.valueVarint32, values.valueVaruint16, values.valueVaruint32, values.valueBool, values.valueCell, values.valueAddress, values.valueStruct);
    ...
    it("replaceGet: should replace values and clear them", async () => {
        for (const { keys } of testCases) {
            ...
            for (const { values } of testCases) {
                // Send the replace operation
                const replaceGetMessage = {
                    $$type: "ReplaceGetAllMaps",
                    ...keys,
                    ...values,
                };
                await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, replaceGetMessage);
                // Retrieve all maps using `allMaps` getter
                const allMaps = await contract.getAllMaps();
                // Iterate over mapConfigs and perform assertions
                mapConfigs.forEach(({ mapName, key, value, keyTransform, valueTransform }) => {
    ...
    it("replaceGet: should return old values when replaced and null when keys do not exist", async () => {
        for (const { keys, values } of testCases) {
            ...
            await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, setMessage);
            // Call the .replace operation on all maps
            const replaceGetResultAfterSet = await contract.getReplaceGetAllMaps(keys.keyInt, keys.keyInt8, keys.keyInt42, keys.keyInt256, keys.keyUint8, keys.keyUint42, keys.keyUint256, keys.keyAddress, values.valueVarint16, values.valueVarint32, values.valueVaruint16, values.valueVaruint32, values.valueBool, values.valueCell, values.valueAddress, values.valueStruct);
    ...
    it("checkNullReference: should throw an error in receiver when accessing a null reference", async () => {
        const result = await contract.send(treasury.getSender(), { value: (0, core_1.toNano)("1") }, {
