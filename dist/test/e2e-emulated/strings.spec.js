...
const strings_StringsTester_1 = require("./contracts/output/strings_StringsTester");
...
describe("strings", () => {
    ...
    beforeEach(async () => {
        ...
        contract = blockchain.openContract(await strings_StringsTester_1.StringsTester.fromInit());
    ...
    it("should implement strings correctly", async () => {
        ...
        expect((await contract.getDynamicStringCell()).equals((0, core_1.beginCell)().storeStringTail("Hello!").endCell())).toBe(true);
        expect((await contract.getDynamicStringCell2()).equals((0, core_1.beginCell)().storeStringTail("Hello, World!").endCell())).toBe(true);
        expect((await contract.getDynamicCommentCell()).equals((0, core_1.beginCell)()
            .storeUint(0, 32)
        ...
        expect((await contract.getDynamicCommentCellLarge()).equals((0, core_1.beginCell)().storeStringTail(l2).endCell())).toBe(true);
        ...
        expect(await contract.getIntToString(-(2n ** 256n) + 1n)).toEqual((-(2n ** 256n) + 1n).toString());
        expect(await contract.getIntToString(2n ** 256n - 1n)).toEqual((2n ** 256n - 1n).toString());
        function toFloatString(x, digits) {
            const sign = x < 0n ? "-" : "";
            const xAbs = x < 0n ? -x : x;
        ...
        for (let x = -100n; x < 100n; x++) {
            for (let digits = 1; digits < 5; digits++) {
                expect(await contract.getFloatToString(x, BigInt(digits))).toEqual(toFloatString(x, digits));
            }
        }
        await expect(contract.getFloatToString(-(2n ** 256n), 2n)).rejects.toThrow(); // algorithm works with positive numbers so when negating -2^256 we get 2^256 which is out of range
        ...
        expect(await contract.getFloatToString(123n, 77n)).toEqual("0." + "0".repeat(74) + "123");
        ...
        expect(base.loadBuffer(base.remainingBits / 8).toString()).toEqual("Many hands make light work.");
        const b64cases = [
            "SGVsbG8gV29ybGQ=",
        ...
        expect(await contract.getStringWithEscapedChars1()).toBe('test \n \n \\ \\\n "string"');
        expect(await contract.getStringWithEscapedChars2()).toEqual('test \n test \t test \r test \b test \f test " test \' test \\ \\\\ "_" "" test');
        expect(await contract.getStringWithEscapedChars3()).toEqual('test \\n test \\t test \\r test \\\\b\b test \\f test \\" test \\\' test \v \v \\\\ \\\\\\\\ \\"_\\" \\"\\" test');
        expect(await contract.getStringWithEscapedChars4()).toEqual("\u{2028}\u{2029} \u0044 \x41\x42\x43");
        expect(await contract.getStringWithEscapedChars5()).toEqual("\u{0} \u{00} \u{000} \u{0000} \u{00000} \u{000000} \u0000 \x00");
        expect(await contract.getStringWithEscapedChars6()).toEqual(`\x7F\x1F\x0A\x00 TACT`);
