...
const math_MathTester_1 = require("./contracts/output/math_MathTester");
...
describe("math", () => {
    ...
    beforeEach(async () => {
        ...
        contract = blockchain.openContract(await math_MathTester_1.MathTester.fromInit());
    ...
    it("should perform math operations correctly", async () => {
        const addressA = (0, random_utils_1.randomAddress)(0, "a");
        const addressB = (0, random_utils_1.randomAddress)(0, "b");
        const cellA = (0, core_1.beginCell)().storeUint(0, 32).endCell();
        const cellB = (0, core_1.beginCell)().storeUint(1, 32).endCell();
        const sliceA = (0, core_1.beginCell)()
            .storeBit(0)
            .storeRef((0, core_1.beginCell)().storeBit(1).endCell())
            .endCell()
            .asSlice();
        const sliceB = (0, core_1.beginCell)()
            .storeBit(1)
            .storeRef((0, core_1.beginCell)().storeBit(1).endCell())
            .endCell()
            .asSlice();
        const stringA = "foo";
        const stringB = "bar";
        const dictA = core_1.Dictionary.empty().set(0n, 0n);
        const dictB = core_1.Dictionary.empty().set(0n, 2n);
        // Tests
        ...
        for (let num = 1n; num <= 100n; num++) {
            expect(await contract.getLog2(num)).toBe(BigInt(Math.floor(Math.log2(Number(num)))));
        }
        for (let num = 1n; num <= 10n; num++) {
            for (let base = 2n; base <= 10; base++) {
                const logarithm = BigInt(Math.floor(Math.log2(Number(num)) / Math.log2(Number(base))));
                expect(await contract.getLog(num, base)).toBe(logarithm);
            }
        }
        for (let num = -3n; num <= 3n; num++) {
            if (num <= 0n) {
                await expect(contract.getLog2(num)).rejects.toThrow("Unable to execute get method. Got exit_code: 5");
            }
        }
        for (let num = -3n; num <= 3n; num++) {
            for (let base = -3n; base <= 3n; base++) {
                if (num <= 0n || base <= 1n) {
                    await expect(contract.getLog(num, base)).rejects.toThrow("Unable to execute get method. Got exit_code: 5");
                }
                else {
                    const logarithm = BigInt(Math.floor(Math.log2(Number(num)) / Math.log2(Number(base))));
                    expect(await contract.getLog(num, base)).toBe(logarithm);
        ...
        for (let num = maxint - 10n; num <= maxint; num++) {
            for (let base = 2; base <= 10; base++) {
                expect(await contract.getLog(num, BigInt(base))).toBe(BigInt(num.toString(base).length - 1));
            }
        }
        ...
        await expect(contract.getPow(2n, -1n)).rejects.toThrow("Unable to execute get method. Got exit_code: 5");
        await expect(contract.getPow2(-1n)).rejects.toThrow("Unable to execute get method. Got exit_code: 5");
        ...
        for (let num = 0n; num <= 100n; num++) {
            expect(await contract.getSqrt(num)).toBe(BigInt(Math.round(Math.sqrt(Number(num)))));
        }
        for (let num = -3n; num < 0n; num++) {
            await expect(contract.getSqrt(num)).rejects.toThrow("Unable to execute get method. Got exit_code: 5");
        }
    });
