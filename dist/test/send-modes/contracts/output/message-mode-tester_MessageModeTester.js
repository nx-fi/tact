...
exports.MessageModeTester = exports.MessageModeTester_getterMapping = void 0;
...
function initMessageModeTester_init_args(src) {
    return (builder) => {
        const b_0 = builder;
    };
}
...
const MessageModeTester_errors = {
...
class MessageModeTester {
    ...
    async send(provider, via, args, message) {
        ...
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'AverageResult') {
            body = (0, core_1.beginCell)().store(storeAverageResult(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof core_1.Slice) && message.$$type === 'DoCalculatorRequest') {
            body = (0, core_1.beginCell)().store(storeDoCalculatorRequest(message)).endCell();
        }
        if (body === null) {
    ...
    async getCurrentResult(provider) {
        const builder = new core_1.TupleBuilder();
        const source = (await provider.get(109211, builder.build())).stack;
        const result = source.readBigNumber();
        return result;
    }
}
exports.MessageModeTester = MessageModeTester;
