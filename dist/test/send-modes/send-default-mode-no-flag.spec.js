...
const message_mode_tester_MessageModeTester_1 = require("./contracts/output/message-mode-tester_MessageModeTester");
const message_mode_tester_Calculator_1 = require("./contracts/output/message-mode-tester_Calculator");
...
describe("SendDefaultMode with no flags", () => {
    ...
    beforeEach(async () => {
        ...
        tester = blockchain.openContract(await message_mode_tester_MessageModeTester_1.MessageModeTester.fromInit());
        calculator = blockchain.openContract(await message_mode_tester_Calculator_1.Calculator.fromInit());
        const testerDeployResult = await tester.send(treasure.getSender(), { value: (0, core_1.toNano)("10") }, null);
        ...
        const calculatorDeployResult = await calculator.send(treasure.getSender(), { value: (0, core_1.toNano)("10") }, null);
...
*/
    it("should carry out transactions without errors", async () => {
        ...
        const amountToPayInRequest = (0, core_1.toNano)("1");
        // The amount the calculator contract will pay in its response message to the tester: 1 TON
        const amountToPayInCalculatorResponse = (0, core_1.toNano)("1");
        // Contract balances before all the transactions
        const testerBalanceBefore = (await blockchain.getContract(tester.address)).balance;
        const calculatorBalanceBefore = (await blockchain.getContract(calculator.address)).balance;
        ...
        const { transactions } = await tester.send(treasure.getSender(), { value: (0, core_1.toNano)("10") }, {
        ...
        const testerBalanceAfter = (await blockchain.getContract(tester.address)).balance;
        const calculatorBalanceAfter = (await blockchain.getContract(calculator.address)).balance;
        // Check that the transactions exist
        // Transaction T1 (see summary of transactions at the start of the test)
        const testerRequestTsx = ensureTransactionIsDefined((0, test_utils_1.findTransaction)(transactions, {
            from: treasure.address,
            to: tester.address,
            success: true,
        }));
        // Transaction T2
        const calculatorTsx = ensureTransactionIsDefined((0, test_utils_1.findTransaction)(transactions, {
            from: tester.address,
            to: calculator.address,
            success: true,
        }));
        // Transaction T3
        const testerResultTsx = ensureTransactionIsDefined((0, test_utils_1.findTransaction)(transactions, {
    ...
    */
        ...
        const testerOutMessage = ensureMessageIsDefined(testerRequestTsx.outMessages.get(0));
        // Extract the only message sent by the calculator back to the tester
        const calculatorOutMessage = ensureMessageIsDefined(calculatorTsx.outMessages.get(0));
        ...
        const testerMessageForwardFee = extractTotalMessageForwardFee(testerRequestTsx);
        const testerOutMessageInfo = getMessageInfo(testerOutMessage);
        const expectedTesterOutMessageValue = amountToPayInRequest - testerMessageForwardFee;
        expect(computeErrorIntervalOfNumber(expectedTesterOutMessageValue)).toContain(testerOutMessageInfo.value.toString());
        // Check that the value assigned to calculatorOutMessage is the original "value" in the send function
        // but deducted with the message forward fees.
        const calculatorMessageForwardFee = extractTotalMessageForwardFee(calculatorTsx);
        const calculatorOutMessageInfo = getMessageInfo(calculatorOutMessage);
        const expectedCalculatorOutMessageValue = amountToPayInCalculatorResponse - calculatorMessageForwardFee;
        expect(computeErrorIntervalOfNumber(expectedCalculatorOutMessageValue)).toContain(calculatorOutMessageInfo.value.toString());
        ...
        const testerDelta1 = computeBalanceDelta(testerRequestTsx, testerOutMessageInfo);
        ...
        const calculatorDelta = computeBalanceDelta(calculatorTsx, calculatorOutMessageInfo);
        ...
        const testerDelta2 = computeBalanceDeltaWithNoOutputMessage(testerResultTsx);
        // If we add all the deltas for tester, together with its initial balance, we should get its measured final balance
        const expectedTesterBalanceAfter = testerBalanceBefore + testerDelta1 + testerDelta2;
        expect(computeErrorIntervalOfNumber(expectedTesterBalanceAfter)).toContain(testerBalanceAfter.toString());
        // Similarly for the calculator
        const expectedCalculatorBalanceAfter = calculatorBalanceBefore + calculatorDelta;
        expect(computeErrorIntervalOfNumber(expectedCalculatorBalanceAfter)).toContain(calculatorBalanceAfter.toString());
    ...
    */
    it("should test a request with not enough funds to pay for request message forward fees", async () => {
        ...
        expect((0, test_utils_1.findTransaction)(transactions, {
            from: tester.address,
            to: calculator.address,
        })).toBeUndefined();
        ...
        const testerDelta = computeBalanceDeltaWithNoOutputMessage(testerRequestTsx);
        ...
        expect(calculatorBalanceBefore.toString()).toBe(calculatorBalanceAfter.toString());
        ...
        const incomingMessageInfo = getMessageInfo(ensureMessageIsDefined(testerRequestTsx.inMessage));
        const expectedPayedTotalFees = -(testerBalanceAfter -
            testerBalanceBefore -
            incomingMessageInfo.value);
        expect(computeErrorIntervalOfNumber(expectedPayedTotalFees)).toContain(testerRequestTsx.totalFees.coins.toString());
    });
    ...
    */
    it("should test an invalid request to calculator with enough funds to pay for bounced message forward fees", async () => {
        ...
        const testerBouncedTsx = ensureTransactionIsDefined((0, test_utils_1.findTransaction)(transactions, {
            from: calculator.address,
            to: tester.address,
            success: true,
        }));
        ...
        const calculatorTsxDescription = getTransactionDescription(calculatorTsx);
        ...
        const calculatorInMessage = ensureMessageIsDefined(calculatorTsx.inMessage);
        // Check that the value assigned to testerOutMessage is the original "value" in the send function
        // but deducted with the message forward fees
        const testerMessageForwardFee = extractTotalMessageForwardFee(testerRequestTsx);
        const testerOutMessageInfo = getMessageInfo(testerOutMessage);
        ...
        const calculatorBouncePhaseMessageForwardFees = extractTotalBouncedMessageForwardFee(calculatorTsx);
        const calculatorOutMessageInfo = getMessageInfo(calculatorOutMessage);
        ...
        const expectedCalculatorOutMessageValue = calculatorInMessageInfo.value -
        ...
        expect(computeErrorIntervalOfNumber(calculatorOutMessageInfo.validatorsForwardFee)).toContain(calculatorBouncePhaseMessageForwardFees.toString());
        ...
        // Balance delta for tester in its second transaction
        // Since this transaction does not send messages, we pass no MessageInfo object
        const testerDelta2 = computeBalanceDeltaWithNoOutputMessage(testerBouncedTsx);
        // If we add all the deltas for tester, together with its initial balance, we should get its measured final balance
        const expectedTesterBalanceAfter = testerBalanceBefore + testerDelta1 + testerDelta2;
        expect(computeErrorIntervalOfNumber(expectedTesterBalanceAfter)).toContain(testerBalanceAfter.toString());
        // Similarly for the calculator
    ...
    */
    it("should test error in the calculator's computation phase, but no funds to send the bounced message", async () => {
        ...
        const amountToPayInRequest = (0, core_1.toNano)("0.004");
        ...
        const expectedRemainingAmount = calculatorInMessageInfo.value - calculatorTsx.totalFees.coins;
        expect(computeErrorIntervalOfNumber(expectedRemainingAmount)).toContain("0");
        // Now we check that the observed final balances in each contract can actually be obtained from their initial balances
        // by subtracting the transaction fees, crediting the initial message value, and subtracting the outbound message values.
        // In other words, we want to check that the relation C_B + D = C_A holds, where C_B is the contract balance BEFORE the transaction,
        // C_A is the contract balance AFTER the transaction, and D is the "delta" amount encoding the transaction fees and similar quantities.
        ...
        const testerDelta = computeBalanceDelta(testerRequestTsx, testerOutMessageInfo);
        ...
        const calculatorDelta = computeBalanceDeltaWithNoOutputMessage(calculatorTsx);
...
function computeBalanceDeltaWithNoOutputMessage(tsx) {
...
function computeBalanceDelta(tsx, outMsgInfo) {
...
function extractTotalMessageForwardFee(tsx) {
...
function extractTotalBouncedMessageForwardFee(tsx) {
    if (tsx.description.type === "generic") {
...
function getComputationPhase(tsx) {
    if (tsx.description.type === "generic") {
        ...
        if (compPhase.type === "vm") {
            ...
            throw new Error("Computation phase was expected to execute (i.e. not skipped)");
        }
    }
    throw new Error("Unrecognized transaction type");
}
function getActionPhase(tsx) {
    if (tsx.description.type === "generic") {
...
function computeErrorIntervalOfNumber(num) {
