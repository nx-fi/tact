...
const withContext = (ctx, callback) => {
    try {
        context = ctx;
        return callback();
    }
...
function createRef(s) {
    if (context.origin === null) {
        (0, errors_1.throwInternalCompilerError)("Parser context was not initialized");
    }
    return (0, src_info_1.getSrcInfoFromOhm)(s.source, context.currentFile, context.origin);
}
...
function checkVariableName(name, loc) {
...
const checkAttributes = (kind) => (isAbstract, attributes, loc) => {
...
function parseImportString(importText, loc) {
    ...
    if (guessedPath.startsWith(stdlibPrefix)) {
        ...
        return { path: (0, path_1.fromString)(guessedPath), type: "relative", language };
    }
    else {
        err().invalidImport()(loc);
        return { path: path_1.emptyPath, type: "relative", language: "tact" };
    }
}
semantics.addOperation("astOfImport", {
    Import(_importKwd, path, _semicolon) {
        const stringLiteral = path.astOfExpression();
...
semantics.addOperation("astOfModuleItem", {
    ...
    StructDecl_message(_messageKwd, _optLparen, optIntMsgId, _optRparen, typeId, _lbrace, fields, _rbrace) {
        ...
        return createNode({
            ...
            name: typeId.astOfType(),
            fields: fields.astsOfList(),
            opcode: unwrapOptNode(optIntMsgId, (msgId) => msgId.astOfExpression()),
            loc: createRef(this),
        });
    },
...
semantics.addOperation("astOfItem", {
    ...
    AsmFunction(_asmKwd, optAsmShuffle, funAttributes, _funKwd, funId, funParameters, _optColon, optReturnType, _lbrace, asmInstructions, _rbrace) {
        ...
        return createNode({
            ...
            return: unwrapOptNode(optReturnType, (t) => t.astOfType()),
            params: funParameters.astsOfList(),
            instructions: asmInstructions.children.map((s) => s.astOfAsmInstruction()),
            loc: createRef(this),
        });
    },
    ...
    Receiver_regular(receiveKwd, _lparen, optParameter, _rparen, _lbrace, receiverBody, _rbrace) {
    ...
    Receiver_comment(receiveKwd, _lparen, comment, _rparen, _lbrace, receiverBody, _rbrace) {
        return createNode({
            kind: "receiver",
            selector: createNode({
                kind: "internal",
    ...
    Receiver_bounced(bouncedKwd, _lparen, parameter, _rparen, _lbrace, receiverBody, _rbrace) {
        return createNode({
            kind: "receiver",
            selector: createNode({
                kind: "bounce",
    ...
    Receiver_externalRegular(externalKwd, _lparen, optParameter, _rparen, _lbrace, receiverBody, _rbrace) {
        const optParam = optParameter.children[0];
        const subKind = optParam
            ? createNode({
                kind: "simple",
    ...
    Receiver_externalComment(externalKwd, _lparen, comment, _rparen, _lbrace, receiverBody, _rbrace) {
        return createNode({
            kind: "receiver",
            selector: createNode({
                kind: "external",
...
semantics.addOperation("astOfAsmInstruction", {
    asmInstruction(word) {
    ...
    AsmInstruction_internal(_leftBracket, _ws1, instructions, _rightBracket, _ws2) {
        return [
            "[",
    ...
    AsmInstruction_listNoStateCheck(_lbrace, _ws1, instructions, _rbrace, _ws2) {
        return [
            "({)",
            instructions.children.map((s) => s.astOfAsmInstruction()).join(" "),
            "(})",
    ...
    AsmInstruction_string(startQuotationMarkWord, string, _endQuotationMark, _ws) {
...
semantics.addOperation("astOfStatement", {
    ...
    StatementDestruct(_letKwd, typeId, _lparen, identifiers, endOfIdentifiers, _rparen, _equals, expression, _semicolon) {
        return createNode({
            ...
            ignoreUnspecifiedFields: endOfIdentifiers.astOfExpression().ignoreUnspecifiedFields,
            expression: expression.astOfExpression(),
            loc: createRef(this),
        });
    },
...
function baseOfIntLiteral(node) {
 ...
 */
const getParser = (ast) => {
    const errorTypes = (0, parser_error_1.parserErrorSchema)(display_to_string_1.displayToString);
    function parse({ code, origin, path }) {
        return withContext({
            currentFile: path,
            origin,
            ...
            const matchResult = grammar_ohm_bundle_1.default.match(code);
    ...
    function parseExpression(code) {
        return withContext({
            ...
            const matchResult = grammar_ohm_bundle_1.default.match(code, "Expression");
            if (matchResult.failed()) {
    ...
    function parseImports({ code, origin, path }) {
        return withContext({
            currentFile: path,
            origin,
            createNode: ast.createNode,
            ...
            const matchResult = grammar_ohm_bundle_1.default.match(code, "JustImports");
            if (matchResult.failed()) {
                errorTypes.generic(matchResult, path, origin);
            }
    ...
    function parseStatement(code) {
        return withContext({
            ...
            const matchResult = grammar_ohm_bundle_1.default.match(code, "Statement");
            if (matchResult.failed()) {
                errorTypes.generic(matchResult, "", "user");
            }
