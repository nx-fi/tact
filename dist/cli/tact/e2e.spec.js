...
describe("tact --version", () => {
    ...
    testWin("Returns correct stdout", async () => {
        const result = await tact("--version");
        expect(result).toHaveProperty("stdout", expect.stringMatching(/^\d+\.\d+\.\d+\ngit commit: [0-9a-f]+\n$/));
    });
});
const badContracts = [
    [
...
describe("tact foo.tact", () => {
    testWin.each(badContracts)("Compilation of broken contract doesn't contain stacktrace (%s)", async (name, code) => {
        const result = await tact(await codegen.contract(`no-err-${name}`, code));
        expect(result).toHaveProperty("stdout", expect.not.stringMatching(/.*at \w+ \(.*/));
    });
    test.skip.each(badContracts)("Compilation of broken contract contains stacktrace with --verbose 2 (%s)", async (name, code) => {
        const result = await tact(`--verbose 2 ${await codegen.contract(`err-${name}`, code)}`);
        expect(result).toHaveProperty("stdout", expect.stringMatching(/.*at \w+ \(.*/));
    });
});
const goodContract = `
contract Test {
...
describe("tact foo.tact", () => {
    ...
    testWin("Check single-contract compilation with custom output directory", async () => {
        ...
        expect(result).toMatchObject({ kind: "exited", code: 0 });
        const statPromise = (0, promises_1.stat)((0, path_1.join)((0, path_1.dirname)(path), customOutput));
...
describe("tact --config config.json", () => {
    testWin("Complete results", async () => {
        ...
        expect(result).toMatchObject({ kind: "exited", code: 0 });
        const statPromise = (0, promises_1.stat)(r.outputPath("pkg"));
        await expect(statPromise).resolves.not.toThrow();
    });
    testWin("With decompiled binary", async () => {
        ...
        const result = await tact(`--config ${r.config}`);
        expect(result).toMatchObject({ kind: "exited", code: 0 });
        const statPromise = (0, promises_1.stat)(r.outputPath("code.rev.fif"));
...
describe.only("tact -q foo.tact", () => {
    testWin("-q shows errors ", async () => {
        const path = await codegen.contract(`quiet`, `contract Test { x: Int = A }`);
        const result = await tact(`-q ${path}`);
        expect(result.kind === "exited" &&
            result.stderr.includes("Cannot find 'A'")).toBe(true);
