...
function testExpression(original, simplified) {
    it(`should simplify ${original} to ${simplified}`, () => {
        const ast = (0, ast_helpers_1.getAstFactory)();
        const { parseExpression } = (0, grammar_1.getParser)(ast, grammar_2.defaultParser);
        const util = (0, util_1.getAstUtil)(ast);
        const { partiallyEvalExpression } = (0, constEval_1.getOptimizer)(util);
        const originalValue = partiallyEvalExpression(parseExpression(original), new context_1.CompilerContext());
        const simplifiedValue = dummyEval(parseExpression(simplified), ast);
        const areMatching = (0, ast_helpers_1.eqExpressions)(originalValue, simplifiedValue);
        expect(areMatching).toBe(true);
    });
}
function testExpressionWithOptimizer(original, simplified, optimizer) {
    it(`should simplify ${original} to ${simplified}`, () => {
        const ast = (0, ast_helpers_1.getAstFactory)();
        const { parseExpression } = (0, grammar_1.getParser)(ast, grammar_2.defaultParser);
        const originalValue = optimizer.applyRules(dummyEval(parseExpression(original), ast));
        const simplifiedValue = dummyEval(parseExpression(simplified), ast);
        const areMatching = (0, ast_helpers_1.eqExpressions)(originalValue, simplifiedValue);
        expect(areMatching).toBe(true);
    });
...
function dummyEval(ast, astFactory) {
    const cloneNode = astFactory.cloneNode;
    const util = (0, util_1.getAstUtil)(astFactory);
    const recurse = (ast) => {
        switch (ast.kind) {
            case "null":
                return ast;
            ...
            case "op_unary": {
                ...
                if ((0, ast_helpers_1.isLiteral)(newNode.operand)) {
                    return (0, interpreter_1.evalUnaryOp)(ast.op, newNode.operand, ast.loc, util);
                }
                return newNode;
            }
            case "op_binary": {
                ...
                const newNode = cloneNode(copy);
                if ((0, ast_helpers_1.isLiteral)(newNode.left) && (0, ast_helpers_1.isLiteral)(newNode.right)) {
                    const valR = newNode.right;
                    return (0, interpreter_1.evalBinaryOp)(ast.op, newNode.left, () => valR, ast.loc, util);
                }
                return newNode;
            }
            case "conditional": {
...
class ParameterizableDummyOptimizer {
    rules;
    util;
    constructor(rules, Ast) {
        this.util = (0, util_1.getAstUtil)(Ast);
        this.rules = rules;
    }
    applyRules = (ast) => {
        return this.rules.reduce((prev, rule) => rule.applyRule(prev, this), ast);
    };
}
describe("partial-evaluator", () => {
    ...
    associativeRuleExpressions.forEach((test) => {
        ...
        const optimizer = new ParameterizableDummyOptimizer([new associative_1.AssociativeRule3()], (0, ast_helpers_1.getAstFactory)());
