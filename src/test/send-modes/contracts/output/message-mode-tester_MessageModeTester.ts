...
export function loadAverageRequest(slice: Slice) {
    ...
    return {
        $$type: "AverageRequest" as const,
        from: _from,
        to: _to,
        amountToPayInResponse: _amountToPayInResponse,
    };
}
...
function dictValueParserAverageRequest(): DictionaryValue<AverageRequest> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeAverageRequest(src)).endCell(),
            );
        },
        parse: (src) => {
...
function dictValueParserAverageResult(): DictionaryValue<AverageResult> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeAverageResult(src)).endCell(),
            );
        },
        parse: (src) => {
...
export function loadDoCalculatorRequest(slice: Slice) {
    ...
    return {
        $$type: "DoCalculatorRequest" as const,
        from: _from,
        to: _to,
        amountToPayInRequest: _amountToPayInRequest,
        amountToPayInCalculatorResponse: _amountToPayInCalculatorResponse,
    };
}
...
function dictValueParserDoCalculatorRequest(): DictionaryValue<DoCalculatorRequest> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeDoCalculatorRequest(src)).endCell(),
            );
        },
        parse: (src) => {
...
function dictValueParserMessageModeTester$Data(): DictionaryValue<MessageModeTester$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeMessageModeTester$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
type MessageModeTester_init_args = {
    $$type: "MessageModeTester_init_args";
};

function initMessageModeTester_init_args(src: MessageModeTester_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function MessageModeTester_init() {
    const __code = Cell.fromBase64(
        "",
    );
    const builder = beginCell();
    builder.storeUint(0, 1);
    initMessageModeTester_init_args({ $$type: "MessageModeTester_init_args" })(
        builder,
    );
...
const MessageModeTester_errors: { [key: number]: { message: string } } = {
...
const MessageModeTester_types: ABIType[] = [
    {
        name: "DataSize",
        header: null,
        fields: [
    ...
    { name: "Calculator$Data", header: null, fields: [] },
    {
        ...
        fields: [
            ...
            {
                name: "amountToPayInCalculatorResponse",
                type: {
                    kind: "simple",
                    type: "int",
                    optional: false,
...
const MessageModeTester_getters: ABIGetter[] = [
    {
        name: "currentResult",
        methodId: 109211,
        arguments: [],
...
export const MessageModeTester_getterMapping: { [key: string]: string } = {
    currentResult: "getCurrentResult",
};

const MessageModeTester_receivers: ABIReceiver[] = [
    { receiver: "internal", message: { kind: "empty" } },
    { receiver: "internal", message: { kind: "typed", type: "AverageResult" } },
    {
        receiver: "internal",
        message: { kind: "typed", type: "DoCalculatorRequest" },
    },
];

export class MessageModeTester implements Contract {
    static async init() {
        return await MessageModeTester_init();
    }

    static async fromInit() {
        const __gen_init = await MessageModeTester_init();
        const address = contractAddress(0, __gen_init);
        return new MessageModeTester(address, __gen_init);
    }

    static fromAddress(address: Address) {
        return new MessageModeTester(address);
    }

    readonly address: Address;
    readonly init?: { code: Cell; data: Cell };
    readonly abi: ContractABI = {
        types: MessageModeTester_types,
        getters: MessageModeTester_getters,
        receivers: MessageModeTester_receivers,
        errors: MessageModeTester_errors,
    };
    ...
    async send(
        ...
        message: null | AverageResult | DoCalculatorRequest,
    ) {
        ...
        if (
            ...
            message.$$type === "AverageResult"
        ) {
            body = beginCell().store(storeAverageResult(message)).endCell();
        }
        if (
            message &&
            typeof message === "object" &&
            !(message instanceof Slice) &&
            message.$$type === "DoCalculatorRequest"
        ) {
            body = beginCell()
                .store(storeDoCalculatorRequest(message))
                .endCell();
        }
        if (body === null) {
    ...
    async getCurrentResult(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(109211 as any, builder.build()))
            .stack;
