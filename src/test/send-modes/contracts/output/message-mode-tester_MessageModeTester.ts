...
const MessageModeTester_getters: ABIGetter[] = [
    {"name":"currentResult","methodId":109211,"arguments":[],"returnType":{"kind":"simple","type":"int","optional":false,"format":257}},
]
...
const MessageModeTester_receivers: ABIReceiver[] = [
    ...
    {"receiver":"internal","message":{"kind":"typed","type":"DoCalculatorRequest"}},
]
...
export class MessageModeTester implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | AverageResult | DoCalculatorRequest) {
        
        let body: Cell | null = null;
        if (message === null) {
            body = new Cell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'AverageResult') {
            body = beginCell().store(storeAverageResult(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'DoCalculatorRequest') {
            body = beginCell().store(storeDoCalculatorRequest(message)).endCell();
        }
    ...
    async getCurrentResult(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(109211 as any, builder.build())).stack;
