...
function dictValueParserSpanishInquisition(): DictionaryValue<SpanishInquisition> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeSpanishInquisition(src)).endCell(),
            );
        },
        parse: (src) => {
...
export function storeReservedContractErrorsTester$Data(
    src: ReservedContractErrorsTester$Data,
) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadReservedContractErrorsTester$Data(slice: Slice) {
    ...
    return {
        $$type: "ReservedContractErrorsTester$Data" as const,
        owner: _owner,
    };
}
...
function storeTupleReservedContractErrorsTester$Data(
    source: ReservedContractErrorsTester$Data,
) {
    const builder = new TupleBuilder();
...
function dictValueParserReservedContractErrorsTester$Data(): DictionaryValue<ReservedContractErrorsTester$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell()
                    .store(storeReservedContractErrorsTester$Data(src))
                    .endCell(),
            );
        },
        parse: (src) => {
            return loadReservedContractErrorsTester$Data(
...
function initReservedContractErrorsTester_init_args(
    src: ReservedContractErrorsTester_init_args,
) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
...
const ReservedContractErrorsTester_errors: {
    ...
    20158: {
        message: `dnsInternalNormalize() didn't error on Slice with refs`,
    },
    24161: { message: `Invalid DNS name` },
    25189: { message: `Slice.fromBase64() didn't error on invalid Base64` },
    28760: {
        message: `Slice.asAddress() didn't error on invalid tag prefix for a masterchain address`,
    },
    37468: {
        message: `Slice.asAddress() didn't error on invalid account ID size for a basechain address`,
    },
    41357: {
        message: `Slice.asAddress() didn't error on invalid tag prefix for a basechain address`,
    },
    43850: { message: `Int.toFloatString() didn't error on digits 78` },
    46964: { message: `String.fromBase64() didn't error on invalid Base64` },
    49334: {
        message: `Slice.asAddress() didn't error on invalid account ID size for a masterchain address`,
    },
    53355: {
        message: `Slice.asAddress() didn't error on invalid account ID size for a workchain address`,
    },
    60204: { message: `Int.toFloatString() didn't error on digits -1` },
    61605: {
        message: `Slice.asAddress() didn't error on invalid tag prefix for a workchain address`,
    },
};
...
const ReservedContractErrorsTester_types: ABIType[] = [
    ...
    {
        name: "ReservedContractErrorsTester$Data",
        header: null,
        fields: [
            {
                name: "owner",
...
export const ReservedContractErrorsTester_getterMapping: {
    [key: string]: string;
} = {
    owner: "getOwner",
};
...
export class ReservedContractErrorsTester implements Contract {
    ...
    async send(
        ...
        message: null | "128" | "130" | "132" | "134" | "136",
    ) {
