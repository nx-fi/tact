...
function dictValueParserFunctions$Data(): DictionaryValue<Functions$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeFunctions$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
async function Functions_init() {
    const __code = Cell.fromBase64(
        "te6ccgEBBQEArQABFP8A9KQT9LzyyAsBA8DTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+CAINcFb6MwIYIQMZTkNLqOojGBAQHXAAExggDZ7SHCAPL02zzIfwHKAAEBgQEBzwDJ7VTgAYIQnWBa67rjAlvywIICBAMAIu1E0NIAAZeBAQHXAAEx4DBwAUSBAQHXAAExggDZ7SHCAPL0o9s8yH8BygABAYEBAc8Aye1UBAACoA==",
    );
