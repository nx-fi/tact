...
function loadGetterTupleSub(source: TupleReader) {
    const _value = source.readBigNumber();
    return { $$type: 'Sub' as const, value: _value };
}
...
function dictValueParserSub(): DictionaryValue<Sub> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSub(src)).endCell());
        },
        parse: (src) => {
...
export function storeFunctions$Data(src: Functions$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeInt(src.value, 257);
    };
...
export function loadFunctions$Data(slice: Slice) {
    const sc_0 = slice;
    const _value = sc_0.loadIntBig(257);
    return { $$type: 'Functions$Data' as const, value: _value };
}

function loadTupleFunctions$Data(source: TupleReader) {
    const _value = source.readBigNumber();
    return { $$type: 'Functions$Data' as const, value: _value };
}

function loadGetterTupleFunctions$Data(source: TupleReader) {
    const _value = source.readBigNumber();
    return { $$type: 'Functions$Data' as const, value: _value };
}

function storeTupleFunctions$Data(source: Functions$Data) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.value);
    return builder.build();
}

function dictValueParserFunctions$Data(): DictionaryValue<Functions$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeFunctions$Data(src)).endCell());
        },
        parse: (src) => {
...
async function Functions_init() {
    const __code = Cell.fromBase64('te6ccgEBBQEArQABFP8A9KQT9LzyyAsBA8DTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+CAINcFb6MwIYIQMZTkNLqOojGBAQHXAAExggDZ7SHCAPL02zzIfwHKAAEBgQEBzwDJ7VTgAYIQnWBa67rjAlvywIICBAMAIu1E0NIAAZeBAQHXAAEx4DBwAUSBAQHXAAExggDZ7SHCAPL0o9s8yH8BygABAYEBAc8Aye1UBAACoA==');
...
export const Functions_getterMapping: { [key: string]: string } = {
}
