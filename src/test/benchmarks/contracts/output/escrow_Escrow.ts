...
export function loadUpdateJettonWalletCode(slice: Slice) {
    ...
    return {
        $$type: "UpdateJettonWalletCode" as const,
        newJettonWalletCode: _newJettonWalletCode,
    };
}
...
function dictValueParserUpdateJettonWalletCode(): DictionaryValue<UpdateJettonWalletCode> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeUpdateJettonWalletCode(src)).endCell(),
            );
        },
        parse: (src) => {
...
function dictValueParserTakeEscrowData(): DictionaryValue<TakeEscrowData> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeTakeEscrowData(src)).endCell(),
            );
        },
        parse: (src) => {
...
export function storeEscrowData(src: EscrowData) {
    return (builder: Builder) => {
        ...
        if (
            src.jettonWalletCode !== null &&
            src.jettonWalletCode !== undefined
        ) {
...
export function loadEscrowData(slice: Slice) {
    ...
    return {
        $$type: "EscrowData" as const,
        id: _id,
        sellerAddress: _sellerAddress,
        guarantorAddress: _guarantorAddress,
        dealAmount: _dealAmount,
        guarantorRoyaltyPercent: _guarantorRoyaltyPercent,
        isFunded: _isFunded,
...
function dictValueParserEscrow$Data(): DictionaryValue<Escrow$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeEscrow$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
const Escrow_types: ABIType[] = [
    ...
    {
        name: "UpdateJettonWalletCode",
        header: 492442125,
        fields: [
            {
                name: "newJettonWalletCode",
                type: { kind: "simple", type: "cell", optional: false },
            },
        ],
    },
    {
        ...
        fields: [
            {
                name: "escrowData",
                type: { kind: "simple", type: "EscrowData", optional: false },
            },
        ],
    },
    {
...
const Escrow_getters: ABIGetter[] = [
    ...
    },
    {
        ...
        methodId: 94138,
        arguments: [],
        returnType: { kind: "simple", type: "EscrowData", optional: false },
    },
];
...
const Escrow_receivers: ABIReceiver[] = [
    ...
    {
        receiver: "internal",
        message: { kind: "typed", type: "UpdateJettonWalletCode" },
    },
    {
        receiver: "internal",
        message: { kind: "typed", type: "JettonNotification" },
    },
    { receiver: "internal", message: { kind: "text", text: "approve" } },
    { receiver: "internal", message: { kind: "text", text: "cancel" } },
    {
        receiver: "internal",
        message: { kind: "text", text: "provideEscrowData" },
    },
...
export class Escrow implements Contract {
    ...
    static async fromInit(
        ...
        const __gen_init = await Escrow_init(
            id,
            sellerAddress,
            guarantorAddress,
            dealAmount,
    ...
    async send(
        ...
        if (
            ...
            message.$$type === "UpdateJettonWalletCode"
        ) {
            body = beginCell()
                .store(storeUpdateJettonWalletCode(message))
                .endCell();
        }
        if (
            message &&
            typeof message === "object" &&
            !(message instanceof Slice) &&
            message.$$type === "JettonNotification"
        ) {
            body = beginCell()
                .store(storeJettonNotification(message))
                .endCell();
        }
        if (message === "approve") {
            body = beginCell()
    ...
    async getCalculateRoyaltyAmount(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(78929 as any, builder.build()))
            .stack;
    ...
    async getWalletAddress(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(129664 as any, builder.build()))
            .stack;
    ...
    async getEscrowInfo(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(94138 as any, builder.build()))
            .stack;
