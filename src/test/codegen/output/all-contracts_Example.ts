...
function dictValueParserFactoryDeploy(): DictionaryValue<FactoryDeploy> {
 ...
 type Example_init_args = {
    $$type: 'Example_init_args';
}

function initExample_init_args(src: Example_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}
...
async function Example_init() {
    const __code = Cell.fromBase64('te6ccgEBAwEAgwABFP8A9KQT9LzyyAsBAc7TAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+CAINcFb6NbIIIIEBAQupowMMh/AcoAye1U4CDAU5owMMh/AcoAye1U4CDALJowMMh/AcoAye1U4MAtmTDIfwHKAMntVOAw8sCCAgAU7UTQ0gAwkW3gbQ==');
...
const Example_errors: { [key: number]: { message: string } } = {
...
const Example_getters: ABIGetter[] = [
]

export const Example_getterMapping: { [key: string]: string } = {
}

const Example_receivers: ABIReceiver[] = [
...
export class Example implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: Binary | Octal | Decimal | Hexadecimal) {
        
        let body: Cell | null = null;
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Binary') {
            body = beginCell().store(storeBinary(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Octal') {
            body = beginCell().store(storeOctal(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Decimal') {
            body = beginCell().store(storeDecimal(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Hexadecimal') {
            body = beginCell().store(storeHexadecimal(message)).endCell();
        }
