...
export const Maps_getterMapping: { [key: string]: string } = {
    'oneItem': 'getOneItem',
...
export class Maps implements Contract {
    ...
    async getOneItem(provider: ContractProvider, key: bigint) {
        ...
        const source = (await provider.get(68660 as any, builder.build())).stack;
    ...
    async getItemCheck(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(102138 as any, builder.build())).stack;
    ...
    async getAllItems(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(68968 as any, builder.build())).stack;
        const result = Dictionary.loadDirect(Dictionary.Keys.Address(), dictValueParserTokenInfo(), source.readCellOpt());
        return result;
    }
