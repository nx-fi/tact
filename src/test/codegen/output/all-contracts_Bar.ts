...
function initBar_init_args(src: Bar_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function Bar_init() {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initBar_init_args({ $$type: 'Bar_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const Bar_errors: { [key: number]: { message: string } } = {
...
const Bar_getters: ABIGetter[] = [
]

export const Bar_getterMapping: { [key: string]: string } = {
}

const Bar_receivers: ABIReceiver[] = [
]

export class Bar implements Contract {
    
    static async init() {
        return await Bar_init();
    }
    
    static async fromInit() {
        const __gen_init = await Bar_init();
        const address = contractAddress(0, __gen_init);
        return new Bar(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new Bar(address);
    }
