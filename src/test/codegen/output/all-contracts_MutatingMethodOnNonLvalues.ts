...
function dictValueParserFactoryDeploy(): DictionaryValue<FactoryDeploy> {
 ...
 type MutatingMethodOnNonLvalues_init_args = {
    $$type: 'MutatingMethodOnNonLvalues_init_args';
}

function initMutatingMethodOnNonLvalues_init_args(src: MutatingMethodOnNonLvalues_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function MutatingMethodOnNonLvalues_init() {
    const __code = Cell.fromBase64('te6ccgEBBwEAmwABFP8A9KQT9LzyyAsBAqbTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+Ag10nCH5PTHzHe+QGC8Pk3KPTjji+n6p05GrWBpOnBhO/e7nnnfbOPAcB6HGJLuuMCMPLAggIDABTtRNDSADCRbeBtBByAKds8gCrbPIAr2zyALAUFBQQBFts8MMh/AcoAye1UBQEG2zwwBgACpA==');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initMutatingMethodOnNonLvalues_init_args({ $$type: 'MutatingMethodOnNonLvalues_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const MutatingMethodOnNonLvalues_errors: { [key: number]: { message: string } } = {
...
const MutatingMethodOnNonLvalues_types: ABIType[] = [
...
const MutatingMethodOnNonLvalues_getters: ABIGetter[] = [
]

export const MutatingMethodOnNonLvalues_getterMapping: { [key: string]: string } = {
}

const MutatingMethodOnNonLvalues_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"text","text":"foo"}},
]

export class MutatingMethodOnNonLvalues implements Contract {
    
    static async init() {
        return await MutatingMethodOnNonLvalues_init();
    }
    
    static async fromInit() {
        const __gen_init = await MutatingMethodOnNonLvalues_init();
        const address = contractAddress(0, __gen_init);
        return new MutatingMethodOnNonLvalues(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new MutatingMethodOnNonLvalues(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  MutatingMethodOnNonLvalues_types,
        getters: MutatingMethodOnNonLvalues_getters,
        receivers: MutatingMethodOnNonLvalues_receivers,
        errors: MutatingMethodOnNonLvalues_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: "foo") {
