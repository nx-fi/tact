...
function dictValueParserFactoryDeploy(): DictionaryValue<FactoryDeploy> {
 ...
 type Issue74_init_args = {
    $$type: 'Issue74_init_args';
}

function initIssue74_init_args(src: Issue74_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function Issue74_init() {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initIssue74_init_args({ $$type: 'Issue74_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const Issue74_errors: { [key: number]: { message: string } } = {
...
const Issue74_getters: ABIGetter[] = [
]

export const Issue74_getterMapping: { [key: string]: string } = {
}

const Issue74_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"text","text":"send"}},
]

export class Issue74 implements Contract {
    
    static async init() {
        return await Issue74_init();
    }
    
    static async fromInit() {
        const __gen_init = await Issue74_init();
        const address = contractAddress(0, __gen_init);
        return new Issue74(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new Issue74(address);
    }
    ...
    readonly abi: ContractABI = {
        types:  Issue74_types,
        getters: Issue74_getters,
        receivers: Issue74_receivers,
        errors: Issue74_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: "send") {
