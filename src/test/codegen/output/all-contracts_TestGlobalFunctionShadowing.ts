...
function dictValueParserFactoryDeploy(): DictionaryValue<FactoryDeploy> {
 ...
 type TestGlobalFunctionShadowing_init_args = {
    $$type: 'TestGlobalFunctionShadowing_init_args';
}

function initTestGlobalFunctionShadowing_init_args(src: TestGlobalFunctionShadowing_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function TestGlobalFunctionShadowing_init() {
    const __code = Cell.fromBase64('te6ccgEBAwEAeAABFP8A9KQT9LzyyAsBAbjTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+Ag10nCH5PTHzHe+QGC8Pk3KPTjji+n6p05GrWBpOnBhO/e7nnnfbOPAcB6HGJLupkwyH8BygDJ7VTgMPLAggIAFO1E0NIAMJFt4G0=');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initTestGlobalFunctionShadowing_init_args({ $$type: 'TestGlobalFunctionShadowing_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const TestGlobalFunctionShadowing_errors: { [key: number]: { message: string } } = {
...
const TestGlobalFunctionShadowing_types: ABIType[] = [
...
const TestGlobalFunctionShadowing_getters: ABIGetter[] = [
]

export const TestGlobalFunctionShadowing_getterMapping: { [key: string]: string } = {
}

const TestGlobalFunctionShadowing_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"text","text":"foo"}},
]

export class TestGlobalFunctionShadowing implements Contract {
    
    static async init() {
        return await TestGlobalFunctionShadowing_init();
    }
    
    static async fromInit() {
        const __gen_init = await TestGlobalFunctionShadowing_init();
        const address = contractAddress(0, __gen_init);
        return new TestGlobalFunctionShadowing(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new TestGlobalFunctionShadowing(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  TestGlobalFunctionShadowing_types,
        getters: TestGlobalFunctionShadowing_getters,
        receivers: TestGlobalFunctionShadowing_receivers,
        errors: TestGlobalFunctionShadowing_errors,
    };
