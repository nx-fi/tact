...
async function TestContract_init() {
    const __code = Cell.fromBase64('te6ccgEBBgEAbwABbP8AII4oMDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gAwkW2RbeIw3PLAguH0pBP0vPLICwECAnECAwEjvcLHaiaGkAGEi2yLbxAO2eGMBAEjvfVfaiaGkAGEi2yLbxAO2eGMBQACbgAEbrM=');
...
export const TestContract_getterMapping: { [key: string]: string } = {
    'testEq': 'getTestEq',
    'testNeq': 'getTestNeq',
}
...
export class TestContract implements Contract {
    ...
    async getTestEq(provider: ContractProvider, s: Dictionary<bigint, bigint>) {
        const builder = new TupleBuilder();
        builder.writeCell(s.size > 0 ? beginCell().storeDictDirect(s, Dictionary.Keys.BigInt(257), Dictionary.Values.BigInt(257)).endCell() : null);
        const source = (await provider.get(79960 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getTestNeq(provider: ContractProvider, s: Dictionary<bigint, bigint>) {
        const builder = new TupleBuilder();
        builder.writeCell(s.size > 0 ? beginCell().storeDictDirect(s, Dictionary.Keys.BigInt(257), Dictionary.Values.BigInt(257)).endCell() : null);
        const source = (await provider.get(114347 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
