...
function initC6fn_init_args(src: C6fn_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C6fn_init(v: bigint) {
    ...
    initC6fn_init_args({ $$type: 'C6fn_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C6fn_errors: { [key: number]: { message: string } } = {
...
export const C6fn_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C6fn_receivers: ABIReceiver[] = [
]


export class C6fn implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C6fn_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C6fn_init(v);
        const address = contractAddress(0, __gen_init);
        return new C6fn(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C6fn(address);
    }
    ...
    readonly abi: ContractABI = {
        types:  C6fn_types,
        getters: C6fn_getters,
        receivers: C6fn_receivers,
        errors: C6fn_errors,
    };
