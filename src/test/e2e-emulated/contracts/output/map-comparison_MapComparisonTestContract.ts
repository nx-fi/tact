...
export function storeCompare(src: Compare) {
    return (builder: Builder) => {
        ...
        b_0.storeDict(src.m1, Dictionary.Keys.Uint(8), Dictionary.Values.Bool());
        b_0.storeDict(src.m2, Dictionary.Keys.Uint(8), Dictionary.Values.Bool());
    };
}

export function loadCompare(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 1473646961) { throw Error('Invalid prefix'); }
    const _m1 = Dictionary.load(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), sc_0);
    const _m2 = Dictionary.load(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), sc_0);
    return { $$type: 'Compare' as const, m1: _m1, m2: _m2 };
}

function loadTupleCompare(source: TupleReader) {
    const _m1 = Dictionary.loadDirect(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), source.readCellOpt());
    const _m2 = Dictionary.loadDirect(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), source.readCellOpt());
    return { $$type: 'Compare' as const, m1: _m1, m2: _m2 };
}
...
function storeTupleCompare(source: Compare) {
    const builder = new TupleBuilder();
    builder.writeCell(source.m1.size > 0 ? beginCell().storeDictDirect(source.m1, Dictionary.Keys.Uint(8), Dictionary.Values.Bool()).endCell() : null);
    builder.writeCell(source.m2.size > 0 ? beginCell().storeDictDirect(source.m2, Dictionary.Keys.Uint(8), Dictionary.Values.Bool()).endCell() : null);
    return builder.build();
}
...
export function loadCompareDeep(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3800136638) { throw Error('Invalid prefix'); }
    const _m1 = Dictionary.load(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), sc_0);
    const _m2 = Dictionary.load(Dictionary.Keys.Uint(8), Dictionary.Values.Bool(), sc_0);
...
function dictValueParserCompareDeep(): DictionaryValue<CompareDeep> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeCompareDeep(src)).endCell());
        },
        parse: (src) => {
...
export function storeMapComparisonTestContract$Data(src: MapComparisonTestContract$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}
...
function storeTupleMapComparisonTestContract$Data(source: MapComparisonTestContract$Data) {
    const builder = new TupleBuilder();
    return builder.build();
}
...
function dictValueParserMapComparisonTestContract$Data(): DictionaryValue<MapComparisonTestContract$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeMapComparisonTestContract$Data(src)).endCell());
        },
        parse: (src) => {
...
function initMapComparisonTestContract_init_args(src: MapComparisonTestContract_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}
...
async function MapComparisonTestContract_init() {
    const __code = Cell.fromBase64('');
...
export const MapComparisonTestContract_getterMapping: { [key: string]: string } = {
...
export class MapComparisonTestContract implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | Compare | CompareDeep) {
        
        let body: Cell | null = null;
        if (message === null) {
            body = new Cell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'Compare') {
            body = beginCell().store(storeCompare(message)).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'CompareDeep') {
    ...
    async getCompareIntInt(provider: ContractProvider, m1: Dictionary<bigint, bigint>, m2: Dictionary<bigint, bigint>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.BigInt(257), Dictionary.Values.BigInt(257)).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.BigInt(257), Dictionary.Values.BigInt(257)).endCell() : null);
        const source = (await provider.get(69033 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getCompareIntCell(provider: ContractProvider, m1: Dictionary<bigint, Cell>, m2: Dictionary<bigint, Cell>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.BigInt(257), Dictionary.Values.Cell()).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.BigInt(257), Dictionary.Values.Cell()).endCell() : null);
        const source = (await provider.get(116487 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getCompareIntAddress(provider: ContractProvider, m1: Dictionary<bigint, Address>, m2: Dictionary<bigint, Address>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.BigInt(257), Dictionary.Values.Address()).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.BigInt(257), Dictionary.Values.Address()).endCell() : null);
        const source = (await provider.get(116486 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getCompareAddressInt(provider: ContractProvider, m1: Dictionary<Address, bigint>, m2: Dictionary<Address, bigint>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.Address(), Dictionary.Values.BigInt(257)).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.Address(), Dictionary.Values.BigInt(257)).endCell() : null);
        const source = (await provider.get(96411 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getCompareAddressCell(provider: ContractProvider, m1: Dictionary<Address, Cell>, m2: Dictionary<Address, Cell>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.Address(), Dictionary.Values.Cell()).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.Address(), Dictionary.Values.Cell()).endCell() : null);
        const source = (await provider.get(121653 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
    
    async getCompareAddressAddress(provider: ContractProvider, m1: Dictionary<Address, Address>, m2: Dictionary<Address, Address>) {
        const builder = new TupleBuilder();
        builder.writeCell(m1.size > 0 ? beginCell().storeDictDirect(m1, Dictionary.Keys.Address(), Dictionary.Values.Address()).endCell() : null);
        builder.writeCell(m2.size > 0 ? beginCell().storeDictDirect(m2, Dictionary.Keys.Address(), Dictionary.Values.Address()).endCell() : null);
        const source = (await provider.get(103155 as any, builder.build())).stack;
        const result = source.readBoolean();
        return result;
    }
