...
function initC2_init_args(src: C2_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C2_init(v: bigint) {
    ...
    initC2_init_args({ $$type: 'C2_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C2_errors: { [key: number]: { message: string } } = {
...
export const C2_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C2_receivers: ABIReceiver[] = [
]


export class C2 implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C2_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C2_init(v);
        const address = contractAddress(0, __gen_init);
        return new C2(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C2(address);
    }
