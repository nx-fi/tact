...
function initC3f_init_args(src: C3f_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C3f_init(v: bigint) {
    const __code = Cell.fromBase64('te6ccgEBAwEAfwABlv8AII49MDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABnYEBAdcAgQEB1wBZbBKagQEB1wABAdGAZOJb3PLAguH0pBP0vPLICwEBUaZsvbtRNDSAAGdgQEB1wCBAQHXAFlsEpqBAQHXAAEB0YBk4ljbPGwhgAgAGUiCg');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initC3f_init_args({ $$type: 'C3f_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C3f_errors: { [key: number]: { message: string } } = {
...
export const C3f_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C3f_receivers: ABIReceiver[] = [
]


export class C3f implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C3f_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C3f_init(v);
        const address = contractAddress(0, __gen_init);
        return new C3f(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C3f(address);
    }
