...
export type SampleDelayedUpgradeContractV2$Data = {
    $$type: 'SampleDelayedUpgradeContractV2$Data';
    _version: bigint;
    initiatedAt: bigint;
    upgradeInfo: Upgrade;
    owner: Address;
...
export function storeSampleDelayedUpgradeContractV2$Data(src: SampleDelayedUpgradeContractV2$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadSampleDelayedUpgradeContractV2$Data(slice: Slice) {
    const sc_0 = slice;
    ...
    return { $$type: 'SampleDelayedUpgradeContractV2$Data' as const, _version: __version, initiatedAt: _initiatedAt, upgradeInfo: _upgradeInfo, owner: _owner, counter: _counter };
}

function loadTupleSampleDelayedUpgradeContractV2$Data(source: TupleReader) {
...
function loadGetterTupleSampleDelayedUpgradeContractV2$Data(source: TupleReader) {
    const __version = source.readBigNumber();
    const _initiatedAt = source.readBigNumber();
...
function storeTupleSampleDelayedUpgradeContractV2$Data(source: SampleDelayedUpgradeContractV2$Data) {
    const builder = new TupleBuilder();
...
function dictValueParserSampleDelayedUpgradeContractV2$Data(): DictionaryValue<SampleDelayedUpgradeContractV2$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSampleDelayedUpgradeContractV2$Data(src)).endCell());
        },
        parse: (src) => {
            return loadSampleDelayedUpgradeContractV2$Data(src.loadRef().beginParse());
        }
    }
}

 type SampleDelayedUpgradeContractV2_init_args = {
    $$type: 'SampleDelayedUpgradeContractV2_init_args';
    owner: Address;
}

function initSampleDelayedUpgradeContractV2_init_args(src: SampleDelayedUpgradeContractV2_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
    };
...
async function SampleDelayedUpgradeContractV2_init(owner: Address) {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initSampleDelayedUpgradeContractV2_init_args({ $$type: 'SampleDelayedUpgradeContractV2_init_args', owner })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const SampleDelayedUpgradeContractV2_errors: { [key: number]: { message: string } } = {
...
const SampleDelayedUpgradeContractV2_types: ABIType[] = [
...
export const SampleDelayedUpgradeContractV2_getterMapping: { [key: string]: string } = {
    'counter': 'getCounter',
...
const SampleDelayedUpgradeContractV2_receivers: ABIReceiver[] = [
...
export class SampleDelayedUpgradeContractV2 implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(owner: Address) {
        return await SampleDelayedUpgradeContractV2_init(owner);
    }
    
    static async fromInit(owner: Address) {
        const __gen_init = await SampleDelayedUpgradeContractV2_init(owner);
        const address = contractAddress(0, __gen_init);
        return new SampleDelayedUpgradeContractV2(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new SampleDelayedUpgradeContractV2(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  SampleDelayedUpgradeContractV2_types,
        getters: SampleDelayedUpgradeContractV2_getters,
        receivers: SampleDelayedUpgradeContractV2_receivers,
        errors: SampleDelayedUpgradeContractV2_errors,
    };
