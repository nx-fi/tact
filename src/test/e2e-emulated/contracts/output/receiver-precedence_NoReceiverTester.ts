...
function dictValueParserAllBouncedTester$Data(): DictionaryValue<AllBouncedTester$Data> {
 ...
 type NoReceiverTester_init_args = {
    $$type: 'NoReceiverTester_init_args';
}

function initNoReceiverTester_init_args(src: NoReceiverTester_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function NoReceiverTester_init() {
    const __code = Cell.fromBase64('te6ccgEBAwEAaQABhP8AII40MDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABlNQB0DGaMIt3Vua25vd26OIw3PLAguH0pBP0vPLICwEBO6ZNGPtRNDSAAGU1AHQMZowi3dW5rbm93bo4ts8MYAIAAiA=');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initNoReceiverTester_init_args({ $$type: 'NoReceiverTester_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const NoReceiverTester_errors: { [key: number]: { message: string } } = {
...
const NoReceiverTester_types: ABIType[] = [
...
const NoReceiverTester_getters: ABIGetter[] = [
    {"name":"receiver","methodId":78947,"arguments":[],"returnType":{"kind":"simple","type":"string","optional":false}},
]

export const NoReceiverTester_getterMapping: { [key: string]: string } = {
    'receiver': 'getReceiver',
}

const NoReceiverTester_receivers: ABIReceiver[] = [
]


export class NoReceiverTester implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init() {
        return await NoReceiverTester_init();
    }
    
    static async fromInit() {
        const __gen_init = await NoReceiverTester_init();
        const address = contractAddress(0, __gen_init);
        return new NoReceiverTester(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new NoReceiverTester(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  NoReceiverTester_types,
        getters: NoReceiverTester_getters,
        receivers: NoReceiverTester_receivers,
        errors: NoReceiverTester_errors,
    };
