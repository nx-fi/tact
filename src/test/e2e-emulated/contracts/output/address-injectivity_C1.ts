...
function initC1_init_args(src: C1_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C1_init(v: bigint) {
    ...
    initC1_init_args({ $$type: 'C1_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C1_errors: { [key: number]: { message: string } } = {
...
export const C1_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C1_receivers: ABIReceiver[] = [
]


export class C1 implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C1_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C1_init(v);
        const address = contractAddress(0, __gen_init);
        return new C1(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C1(address);
    }
