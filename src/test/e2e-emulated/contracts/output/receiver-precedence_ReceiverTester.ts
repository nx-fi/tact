...
export class ReceiverTester implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | string | Message | Slice | "message" | "do_unsupported_op" | "do_div_by_zero" | "do_success_div" | "do_unknown_request" | SendCellToAddress) {
        ...
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'SendCellToAddress') {
            body = beginCell().store(storeSendCellToAddress(message)).endCell();
        }
    ...
    async sendExternal(provider: ContractProvider, message: string | null | 'message' | Message | Slice) {
    ...
    async getReceiverKind(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(87389 as any, builder.build())).stack;
