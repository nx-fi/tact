...
export function loadMessage(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2523316742) { throw Error('Invalid prefix'); }
...
export class ReceiverTester implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | string | Message | Slice | "message" | "do_unsupported_op" | "do_div_by_zero" | "do_success_div" | "do_unknown_request") {
    ...
    async sendExternal(provider: ContractProvider, message: string | null | 'message' | Message) {
