...
type Child7_init_args = {
    $$type: "Child7_init_args";
};

function initChild7_init_args(src: Child7_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}
...
async function Child7_init() {
    const __code = Cell.fromBase64(
        "te6ccgEBAwEAZgABFP8A9KQT9LzyyAsBAYzTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+BwIddJIMIflTEB0x8wkTLiwAABwSGwnMh/AcoAAQHLH8ntVOAw8sCCAgAc7UTQ0gABlNMfATHgMHA=",
    );
...
const Child7_errors: { [key: number]: { message: string } } = {
...
const Child7_getters: ABIGetter[] = [];

export const Child7_getterMapping: { [key: string]: string } = {};

const Child7_receivers: ABIReceiver[] = [
    { receiver: "internal", message: { kind: "empty" } },
];

export class Child7 implements Contract {
    static async init() {
