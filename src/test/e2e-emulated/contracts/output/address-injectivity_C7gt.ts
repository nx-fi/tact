...
function initC7gt_init_args(src: C7gt_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C7gt_init(v: bigint) {
    const __code = Cell.fromBase64('te6ccgEBAwEAbgABhv8AII41MDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABl4EBAdcAATGYgQEB1wABAdHiMNzywILh9KQT9LzyyAsBAT+mbL27UTQ0gABl4EBAdcAATGYgQEB1wABAdHiAds8MYAIABlIQoA==');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initC7gt_init_args({ $$type: 'C7gt_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C7gt_errors: { [key: number]: { message: string } } = {
...
const C7gt_getters: ABIGetter[] = [
    {"name":"incrAndGetField1","methodId":111350,"arguments":[{"name":"t","type":{"kind":"simple","type":"int","optional":false,"format":257}}],"returnType":{"kind":"simple","type":"int","optional":false,"format":257}},
]

export const C7gt_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C7gt_receivers: ABIReceiver[] = [
]


export class C7gt implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C7gt_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C7gt_init(v);
        const address = contractAddress(0, __gen_init);
        return new C7gt(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C7gt(address);
    }
    ...
    readonly abi: ContractABI = {
        types:  C7gt_types,
        getters: C7gt_getters,
        receivers: C7gt_receivers,
        errors: C7gt_errors,
    };
    ...
    async getIncrAndGetField1(provider: ContractProvider, t: bigint) {
        const builder = new TupleBuilder();
        builder.writeNumber(t);
        const source = (await provider.get(111350 as any, builder.build())).stack;
