...
function dictValueParserAllBouncedTester$Data(): DictionaryValue<AllBouncedTester$Data> {
 ...
 type BinaryBouncedTester_init_args = {
    $$type: 'BinaryBouncedTester_init_args';
}

function initBinaryBouncedTester_init_args(src: BinaryBouncedTester_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function BinaryBouncedTester_init() {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initBinaryBouncedTester_init_args({ $$type: 'BinaryBouncedTester_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const BinaryBouncedTester_errors: { [key: number]: { message: string } } = {
...
const BinaryBouncedTester_types: ABIType[] = [
...
const BinaryBouncedTester_getters: ABIGetter[] = [
    {"name":"receiver","methodId":78947,"arguments":[],"returnType":{"kind":"simple","type":"string","optional":false}},
]

export const BinaryBouncedTester_getterMapping: { [key: string]: string } = {
    'receiver': 'getReceiver',
}

const BinaryBouncedTester_receivers: ABIReceiver[] = [
    {"receiver":"internal","message":{"kind":"typed","type":"SendCellToAddress"}},
    {"receiver":"internal","message":{"kind":"text","text":"reset"}},
]


export class BinaryBouncedTester implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init() {
        return await BinaryBouncedTester_init();
    }
    
    static async fromInit() {
        const __gen_init = await BinaryBouncedTester_init();
        const address = contractAddress(0, __gen_init);
        return new BinaryBouncedTester(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new BinaryBouncedTester(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  BinaryBouncedTester_types,
        getters: BinaryBouncedTester_getters,
        receivers: BinaryBouncedTester_receivers,
        errors: BinaryBouncedTester_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: SendCellToAddress | "reset") {
