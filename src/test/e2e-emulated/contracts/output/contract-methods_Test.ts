...
export function storeTest$Data(src: Test$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeUint(src.flags, 256);
        b_0.storeInt(src.oneMoreVar, 257);
    };
}

export function loadTest$Data(slice: Slice) {
    const sc_0 = slice;
    const _flags = sc_0.loadUintBig(256);
    const _oneMoreVar = sc_0.loadIntBig(257);
    return { $$type: 'Test$Data' as const, flags: _flags, oneMoreVar: _oneMoreVar };
}

function loadTupleTest$Data(source: TupleReader) {
    const _flags = source.readBigNumber();
    const _oneMoreVar = source.readBigNumber();
    return { $$type: 'Test$Data' as const, flags: _flags, oneMoreVar: _oneMoreVar };
}
...
function storeTupleTest$Data(source: Test$Data) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.flags);
    builder.writeNumber(source.oneMoreVar);
    return builder.build();
}
...
async function Test_init() {
    const __code = Cell.fromBase64('te6ccgEBBQEAoQABGv8AIOMDbYAT9LzyyAsBA/gwAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLtRNDSAAGa0/+BAQHXAFlsEpUwcIEA7+IDkl8D4HAi10kgwh+VMQLTHzCRM+LAAALBIRKwjywB2zwwggCS6QLAKhLy9HAB2zwwgXrpIsAq8vTIfwHKAFkCy/+BAQHPAMntVOBbAgIDAQTbPAQABvLAggAIgCoyfw==');
...
const Test_errors: { [key: number]: { message: string } } = {
    ...
    31465: { message: `checkReturnViaLocalVar() is mis-compiled` },
    37609: { message: `checkImmediateReturn() is mis-compiled` },
}
...
const Test_types: ABIType[] = [
    ...
    {"name":"Test$Data","header":null,"fields":[{"name":"flags","type":{"kind":"simple","type":"uint","optional":false,"format":256}},{"name":"oneMoreVar","type":{"kind":"simple","type":"int","optional":false,"format":257}}]},
]
