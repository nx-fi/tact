...
function loadGetterTupleSetCost(source: TupleReader) {
    const _cost = source.readBigNumberOpt();
    return { $$type: 'SetCost' as const, cost: _cost };
}
...
export function loadStruct2(slice: Slice) {
    ...
    return { $$type: 'Struct2' as const, c: _c, d: _d, e: _e, f: _f };
}
...
export function storeTest$Data(src: Test$Data) {
    return (builder: Builder) => {
        ...
        if (src.cost !== null && src.cost !== undefined) { b_1.storeBit(true).storeCoins(src.cost); } else { b_1.storeBit(false); }
        b_1.storeAddress(src.address);
        const b_2 = new Builder();
        if (src.price !== null && src.price !== undefined) { b_2.storeBit(true).storeCoins(src.price); } else { b_2.storeBit(false); }
...
export function loadTest$Data(slice: Slice) {
    ...
    return { $$type: 'Test$Data' as const, owner: _owner, seqno: _seqno, a: _a, b: _b, struct2: _struct2, author3: _author3, cost: _cost, address: _address, price: _price };
}
...
async function Test_init(owner: Address, struct2: Struct2) {
    const __code = Cell.fromBase64('');
...
export class Test implements Contract {
    ...
    static async init(owner: Address, struct2: Struct2) {
        return await Test_init(owner, struct2);
    }
    
    static async fromInit(owner: Address, struct2: Struct2) {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: SetCost | Deploy) {
