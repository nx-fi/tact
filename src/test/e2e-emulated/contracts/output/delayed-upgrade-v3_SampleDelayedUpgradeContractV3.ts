...
export function storeSampleDelayedUpgradeContractV3$Data(src: SampleDelayedUpgradeContractV3$Data) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadSampleDelayedUpgradeContractV3$Data(slice: Slice) {
    const sc_0 = slice;
    ...
    return { $$type: 'SampleDelayedUpgradeContractV3$Data' as const, _version: __version, initiatedAt: _initiatedAt, upgradeInfo: _upgradeInfo, owner: _owner, counter: _counter };
}

function loadTupleSampleDelayedUpgradeContractV3$Data(source: TupleReader) {
...
function loadGetterTupleSampleDelayedUpgradeContractV3$Data(source: TupleReader) {
    const __version = source.readBigNumber();
    const _initiatedAt = source.readBigNumber();
...
function storeTupleSampleDelayedUpgradeContractV3$Data(source: SampleDelayedUpgradeContractV3$Data) {
    const builder = new TupleBuilder();
...
function dictValueParserSampleDelayedUpgradeContractV3$Data(): DictionaryValue<SampleDelayedUpgradeContractV3$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeSampleDelayedUpgradeContractV3$Data(src)).endCell());
        },
        parse: (src) => {
 ...
 type SampleDelayedUpgradeContractV3_init_args = {
    $$type: 'SampleDelayedUpgradeContractV3_init_args';
    owner: Address;
}

function initSampleDelayedUpgradeContractV3_init_args(src: SampleDelayedUpgradeContractV3_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
    };
...
async function SampleDelayedUpgradeContractV3_init(owner: Address) {
...
const SampleDelayedUpgradeContractV3_errors: { [key: number]: { message: string } } = {
...
const SampleDelayedUpgradeContractV3_types: ABIType[] = [
...
const SampleDelayedUpgradeContractV3_getters: ABIGetter[] = [
...
export const SampleDelayedUpgradeContractV3_getterMapping: { [key: string]: string } = {
    'counter': 'getCounter',
...
const SampleDelayedUpgradeContractV3_receivers: ABIReceiver[] = [
...
export class SampleDelayedUpgradeContractV3 implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: null | "increment" | "decrement" | Confirm | Upgrade) {
