...
export function loadInit(slice: Slice) {
    ...
    return { $$type: 'Init' as const, foo: _foo };
}
...
function loadGetterTupleInit(source: TupleReader) {
    const _foo = source.readBigNumber();
    return { $$type: 'Init' as const, foo: _foo };
}
...
function dictValueParserInit(): DictionaryValue<Init> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeInit(src)).endCell());
        },
        parse: (src) => {
...
export function loadTest$Data(slice: Slice) {
    ...
    return { $$type: 'Test$Data' as const, foo: _foo };
}
...
async function Test_init(init: Init) {
    const __code = Cell.fromBase64('te6ccgEBBgEAgwABFP8A9KQT9LzyyAsBAgFiAgMBktAB0HLXIdIA0gD6QCEQNFBmbwT4YQL4Yts8ApFb4HAh10kgwh+VMQHTHzCRMuLAAAHBIbCfyH8BygABAYEBAc8Aye1U4DDywIIEAg+hDsW2ebZ4YwQFACjtRNDSAAGXgQEB1wABMeDTBwEB0QACIA==');
...
export class Test implements Contract {
    ...
    static async init(init: Init) {
        return await Test_init(init);
    }
    
    static async fromInit(init: Init) {
    ...
    async getData(provider: ContractProvider) {
        const builder = new TupleBuilder();
        const source = (await provider.get(100194 as any, builder.build())).stack;
