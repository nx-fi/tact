...
function dictValueParserAllBouncedTester$Data(): DictionaryValue<AllBouncedTester$Data> {
 ...
 type CommentAndSliceReceiverTester_init_args = {
    $$type: 'CommentAndSliceReceiverTester_init_args';
}

function initCommentAndSliceReceiverTester_init_args(src: CommentAndSliceReceiverTester_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function CommentAndSliceReceiverTester_init() {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initCommentAndSliceReceiverTester_init_args({ $$type: 'CommentAndSliceReceiverTester_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const CommentAndSliceReceiverTester_errors: { [key: number]: { message: string } } = {
...
const CommentAndSliceReceiverTester_types: ABIType[] = [
...
const CommentAndSliceReceiverTester_getters: ABIGetter[] = [
    {"name":"receiver","methodId":78947,"arguments":[],"returnType":{"kind":"simple","type":"string","optional":false}},
]

export const CommentAndSliceReceiverTester_getterMapping: { [key: string]: string } = {
    'receiver': 'getReceiver',
}

const CommentAndSliceReceiverTester_receivers: ABIReceiver[] = [
...
export class CommentAndSliceReceiverTester implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init() {
        return await CommentAndSliceReceiverTester_init();
    }
    
    static async fromInit() {
        const __gen_init = await CommentAndSliceReceiverTester_init();
        const address = contractAddress(0, __gen_init);
        return new CommentAndSliceReceiverTester(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new CommentAndSliceReceiverTester(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  CommentAndSliceReceiverTester_types,
        getters: CommentAndSliceReceiverTester_getters,
        receivers: CommentAndSliceReceiverTester_receivers,
        errors: CommentAndSliceReceiverTester_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: "message" | Slice) {
    ...
    async sendExternal(provider: ContractProvider, message: 'message' | Slice) {
