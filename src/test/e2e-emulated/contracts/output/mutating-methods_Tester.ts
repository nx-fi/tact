...
export function loadMyStruct(slice: Slice) {
    ...
    return { $$type: 'MyStruct' as const, age: _age };
}
...
export function loadTester$Data(slice: Slice) {
    const sc_0 = slice;
    const _s = sc_0.loadRef().asSlice();
    return { $$type: 'Tester$Data' as const, s: _s };
}
...
export class Tester implements Contract {
    ...
    async getTest10(provider: ContractProvider, dict: Cell | null) {
        const builder = new TupleBuilder();
    ...
    async getTest11(provider: ContractProvider, x: bigint | null) {
        const builder = new TupleBuilder();
