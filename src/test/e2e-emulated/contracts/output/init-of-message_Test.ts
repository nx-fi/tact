...
export function loadInitData(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 2121923400) { throw Error('Invalid prefix'); }
    const _seller = sc_0.loadAddress();
    const _nonce = sc_0.loadUintBig(64);
    return { $$type: 'InitData' as const, seller: _seller, nonce: _nonce };
}

function loadTupleInitData(source: TupleReader) {
    const _seller = source.readAddress();
    const _nonce = source.readBigNumber();
    return { $$type: 'InitData' as const, seller: _seller, nonce: _nonce };
}

function loadGetterTupleInitData(source: TupleReader) {
    const _seller = source.readAddress();
    const _nonce = source.readBigNumber();
    return { $$type: 'InitData' as const, seller: _seller, nonce: _nonce };
}

function storeTupleInitData(source: InitData) {
    const builder = new TupleBuilder();
...
function dictValueParserInitData(): DictionaryValue<InitData> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeInitData(src)).endCell());
        },
        parse: (src) => {
            return loadInitData(src.loadRef().beginParse());
        }
    }
}
...
export function loadTest$Data(slice: Slice) {
    const sc_0 = slice;
    const _seller = sc_0.loadAddress();
    return { $$type: 'Test$Data' as const, seller: _seller };
}
...
function initTest_init_args(src: Test_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.store(storeInitData(src.i));
    };
}

async function Test_init(i: InitData) {
    const __code = Cell.fromBase64('te6ccgEBAwEAeQABFP8A9KQT9LzyyAsBAYrTAdBy1yHSANIA+kAhEDRQZm8E+GEC+GLbPAKRW+BwIddJIMIflTEB0x8wkTLiwAABwSGwm8h/AcoAAc8Wye1U4DDywIICAETtRNDSAAGU+kABMeDTHwGCEH55+0i68uCB+kDTP1kC0QEw');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initTest_init_args({ $$type: 'Test_init_args', i })(builder);
...
const Test_types: ABIType[] = [
    ...
    {"name":"InitData","header":2121923400,"fields":[{"name":"seller","type":{"kind":"simple","type":"address","optional":false}},{"name":"nonce","type":{"kind":"simple","type":"uint","optional":false,"format":64}}]},
    {"name":"Test$Data","header":null,"fields":[{"name":"seller","type":{"kind":"simple","type":"address","optional":false}}]},
]
...
export class Test implements Contract {
    ...
    static async init(i: InitData) {
        return await Test_init(i);
    }
    
    static async fromInit(i: InitData) {
        const __gen_init = await Test_init(i);
