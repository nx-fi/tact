...
function initC4g_init_args(src: C4g_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C4g_init(v: bigint) {
    const __code = Cell.fromBase64('te6ccgEBAwEAbgABhv8AII41MDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABl4EBAdcAATGYgQEB1wABAdHiMNzywILh9KQT9LzyyAsBAT+mQTU7UTQ0gABl4EBAdcAATGYgQEB1wABAdHiAds8MYAIABlIQoA==');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initC4g_init_args({ $$type: 'C4g_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C4g_errors: { [key: number]: { message: string } } = {
...
const C4g_getters: ABIGetter[] = [
    {"name":"incrAndGetField","methodId":66772,"arguments":[{"name":"n","type":{"kind":"simple","type":"int","optional":false,"format":257}}],"returnType":{"kind":"simple","type":"int","optional":false,"format":257}},
]

export const C4g_getterMapping: { [key: string]: string } = {
    'incrAndGetField': 'getIncrAndGetField',
}

const C4g_receivers: ABIReceiver[] = [
]


export class C4g implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C4g_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C4g_init(v);
        const address = contractAddress(0, __gen_init);
        return new C4g(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C4g(address);
    }
    ...
    async getIncrAndGetField(provider: ContractProvider, n: bigint) {
        const builder = new TupleBuilder();
        builder.writeNumber(n);
        const source = (await provider.get(66772 as any, builder.build())).stack;
