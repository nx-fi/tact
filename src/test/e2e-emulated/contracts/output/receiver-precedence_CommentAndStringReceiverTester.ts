...
function dictValueParserAllBouncedTester$Data(): DictionaryValue<AllBouncedTester$Data> {
 ...
 type CommentAndStringReceiverTester_init_args = {
    $$type: 'CommentAndStringReceiverTester_init_args';
}

function initCommentAndStringReceiverTester_init_args(src: CommentAndStringReceiverTester_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
    };
}

async function CommentAndStringReceiverTester_init() {
    const __code = Cell.fromBase64('');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initCommentAndStringReceiverTester_init_args({ $$type: 'CommentAndStringReceiverTester_init_args' })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const CommentAndStringReceiverTester_errors: { [key: number]: { message: string } } = {
...
const CommentAndStringReceiverTester_types: ABIType[] = [
...
const CommentAndStringReceiverTester_getters: ABIGetter[] = [
    {"name":"receiver","methodId":78947,"arguments":[],"returnType":{"kind":"simple","type":"string","optional":false}},
]

export const CommentAndStringReceiverTester_getterMapping: { [key: string]: string } = {
    'receiver': 'getReceiver',
}

const CommentAndStringReceiverTester_receivers: ABIReceiver[] = [
...
export class CommentAndStringReceiverTester implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init() {
        return await CommentAndStringReceiverTester_init();
    }
    
    static async fromInit() {
        const __gen_init = await CommentAndStringReceiverTester_init();
        const address = contractAddress(0, __gen_init);
        return new CommentAndStringReceiverTester(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new CommentAndStringReceiverTester(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  CommentAndStringReceiverTester_types,
        getters: CommentAndStringReceiverTester_getters,
        receivers: CommentAndStringReceiverTester_receivers,
        errors: CommentAndStringReceiverTester_errors,
    };
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: "message" | string) {
    ...
    async sendExternal(provider: ContractProvider, message: 'message' | string) {
