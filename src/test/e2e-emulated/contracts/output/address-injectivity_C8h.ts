...
function initC8h_init_args(src: C8h_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C8h_init(v: bigint) {
    ...
    initC8h_init_args({ $$type: 'C8h_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C8h_errors: { [key: number]: { message: string } } = {
...
export const C8h_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
}

const C8h_receivers: ABIReceiver[] = [
]


export class C8h implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C8h_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C8h_init(v);
        const address = contractAddress(0, __gen_init);
        return new C8h(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C8h(address);
    }
