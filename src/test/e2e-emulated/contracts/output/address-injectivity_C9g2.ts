...
function initC9g2_init_args(src: C9g2_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
...
async function C9g2_init(v: bigint) {
    const __code = Cell.fromBase64('te6ccgEBBgEAsAABlv8AII49MDDQctch0gDSAPpAIRA0UFVvBPhhAfhi7UTQ0gABnYEBAdcAgQEB1wBZbBKagQEB1wABAdGAZOJb3PLAguH0pBP0vPLICwECAnECAwFNvijnaiaGkAAM7AgIDrgECAgOuALLYJTUCAgOuAAIDowDJxbZ42EMBAFPvZe3aiaGkAAM7AgIDrgECAgOuALLYJTUCAgOuAAIDowDJxLG2eNhDAUAAiAABlIgoA==');
    const builder = beginCell();
    builder.storeUint(0, 1);
    initC9g2_init_args({ $$type: 'C9g2_init_args', v })(builder);
    const __data = builder.endCell();
    return { code: __code, data: __data };
}

const C9g2_errors: { [key: number]: { message: string } } = {
...
export const C9g2_getterMapping: { [key: string]: string } = {
    'incrAndGetField1': 'getIncrAndGetField1',
    'getField2': 'getGetField2',
}

const C9g2_receivers: ABIReceiver[] = [
]


export class C9g2 implements Contract {
    
    public static readonly storageReserve = 0n;
    
    static async init(v: bigint) {
        return await C9g2_init(v);
    }
    
    static async fromInit(v: bigint) {
        const __gen_init = await C9g2_init(v);
        const address = contractAddress(0, __gen_init);
        return new C9g2(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new C9g2(address);
    }
    ...
    readonly abi: ContractABI = {
        types:  C9g2_types,
        getters: C9g2_getters,
        receivers: C9g2_receivers,
        errors: C9g2_errors,
    };
    ...
    async getGetField2(provider: ContractProvider) {
        const builder = new TupleBuilder();
