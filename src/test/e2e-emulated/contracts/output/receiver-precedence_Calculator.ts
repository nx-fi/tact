...
async function Calculator_init() {
    const __code = Cell.fromBase64('');
...
export class Calculator implements Contract {
    ...
    async send(provider: ContractProvider, via: Sender, args: { value: bigint, bounce?: boolean| null | undefined }, message: "deploy" | BinaryIntOperation) {
        
        let body: Cell | null = null;
        if (message === "deploy") {
            body = beginCell().storeUint(0, 32).storeStringTail(message).endCell();
        }
        if (message && typeof message === 'object' && !(message instanceof Slice) && message.$$type === 'BinaryIntOperation') {
            body = beginCell().store(storeBinaryIntOperation(message)).endCell();
        }
