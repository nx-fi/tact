...
function dictValueParserDeployParamsMsg(): DictionaryValue<DeployParamsMsg> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeDeployParamsMsg(src)).endCell(),
            );
        },
        parse: (src) => {
...
export function loadDeployComparisonMsg(slice: Slice) {
    ...
    const _body = sc_0;
    return {
        $$type: "DeployComparisonMsg" as const,
        mode: _mode,
        bounce: _bounce,
        contractNum: _contractNum,
        value: _value,
...
function dictValueParserDeployComparisonMsg(): DictionaryValue<DeployComparisonMsg> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeDeployComparisonMsg(src)).endCell(),
            );
        },
        parse: (src) => {
...
export function loadDeployComparisonNoBodyMsg(slice: Slice) {
    ...
    const _value = sc_0.loadCoins();
    return {
        $$type: "DeployComparisonNoBodyMsg" as const,
        mode: _mode,
        bounce: _bounce,
        contractNum: _contractNum,
        value: _value,
...
function storeTupleDeployComparisonNoBodyMsg(
    source: DeployComparisonNoBodyMsg,
) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.mode);
    builder.writeBoolean(source.bounce);
...
function dictValueParserDeployComparisonNoBodyMsg(): DictionaryValue<DeployComparisonNoBodyMsg> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell()
                    .store(storeDeployComparisonNoBodyMsg(src))
                    .endCell(),
            );
        },
        parse: (src) => {
...
export function loadDeployContract$Data(slice: Slice) {
    ...
    return {
        $$type: "DeployContract$Data" as const,
...
function dictValueParserDeployContract$Data(): DictionaryValue<DeployContract$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(
                beginCell().store(storeDeployContract$Data(src)).endCell(),
            );
        },
        parse: (src) => {
...
const DeployContract_types: ABIType[] = [
    ...
    {
        name: "DeployComparisonNoBodyMsg",
        header: 3892793597,
        fields: [
            {
                name: "mode",
...
const DeployContract_receivers: ABIReceiver[] = [
    ...
    {
        receiver: "internal",
        message: { kind: "typed", type: "DeployParamsMsg" },
    },
    {
        receiver: "internal",
        message: { kind: "typed", type: "DeployComparisonMsg" },
    },
    {
        receiver: "internal",
        message: { kind: "typed", type: "DeployComparisonNoBodyMsg" },
    },
];
...
export class DeployContract implements Contract {
    ...
    async send(
        ...
        if (
            ...
            typeof message === "object" &&
            !(message instanceof Slice) &&
            message.$$type === "DeployComparisonMsg"
        ) {
            body = beginCell()
                .store(storeDeployComparisonMsg(message))
                .endCell();
        }
        if (
            message &&
            typeof message === "object" &&
            !(message instanceof Slice) &&
            message.$$type === "DeployComparisonNoBodyMsg"
        ) {
            body = beginCell()
                .store(storeDeployComparisonNoBodyMsg(message))
                .endCell();
        }
        if (body === null) {
